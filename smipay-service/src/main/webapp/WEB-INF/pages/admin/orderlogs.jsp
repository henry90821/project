...
<div style="background-color:#363636;height:50px;color:#fff;font-size:20px;line-height:50px;margin-bottom:0px">
 ...
 <div style="width:90%;line-height:50px;font-size:14px;text-align:left;margin-left: auto;margin-right: auto"">
  ...
  <a href="orderlogquery.do"  style="color:red" >下单日志查询</a>&nbsp;&nbsp; &nbsp;&nbsp;
...
<FORM id=listForm name=userlist method=post action=orderlogquery.do > 
<DIV class=listBar>
  &nbsp;&nbsp; <LABEL>订单来源: </LABEL>
...
<TD ><SPAN>${orderlog.code}</SPAN></TD>
<TD title="${orderlog.msg}"><SPAN>
            <c:if test="${fn:length(orderlog.msg)>'13'}">  
                    ${fn:substring(orderlog.msg,0,13)}...  
            </c:if>  
            <c:if test="${fn:length(orderlog.msg)<='13'}">  
                ${orderlog.msg}  
            </c:if>   
...
<DIV class=pagerBar> 
下单日志&nbsp;总记录数:${page.total} (共${page.pages}页)
<DIV class=pager>
<SPAN id=pager>
</SPAN>
