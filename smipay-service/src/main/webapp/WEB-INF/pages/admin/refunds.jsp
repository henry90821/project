...
<div style="width:90%;line-height:50px;font-size:14px;text-align:left;margin-left: auto;margin-right: auto"">
  ...
  <a href="refundquery.do"  style="color:red">退款记录查询</a>&nbsp;&nbsp; &nbsp;&nbsp; 
...
<FORM id=listForm name=userlist method=post action=refundquery.do > 
<DIV class=listBar>
  &nbsp;&nbsp; <LABEL>订单来源: </LABEL>
...
<TD ><SPAN> 
         <c:if test="${refund.status == 0}">  
          ...
          <c:if test="${empty refund.status}">  
              未处理    
         </c:if>  
</SPAN></TD>
<TD title="${refund.callBackMemo}"><SPAN>
            <c:if test="${fn:length(refund.callBackMemo)>'13'}">  
                    ${fn:substring(refund.callBackMemo,0,13)}...  
            </c:if>  
            <c:if test="${fn:length(refund.callBackMemo)<='13'}">  
                ${refund.callBackMemo}  
            </c:if>   
  ...
  <TD ><SPAN> 
         <c:if test="${refund.noticeStatus == 0}">  
              成功      
         </c:if>  
          <c:if test="${refund.noticeStatus == 1}">  
              失败      
         </c:if>  
...
<DIV class=pagerBar> 
退款交易&nbsp;总记录数:${page.total} (共${page.pages}页)
<DIV class=pager>
<SPAN id=pager>
</SPAN>
