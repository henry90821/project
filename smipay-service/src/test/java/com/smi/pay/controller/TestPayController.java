...
public class TestPayController {
	...
	@RequestMapping ( "/wxlogin" )  //公众号登录获取CODE测试
	...
	@RequestMapping ( "/getopenid" )  //公众号登录获取后获取openid测试
    public ModelAndView getopenid(HttpServletRequest request, 
            HttpServletResponse response) throws IOException{	
       return wxPayService.getopenid(request, response);
    }
	@ResponseBody
	@RequestMapping ( "/wxpay" )  //公众号登录获取后获取openid测试
    public ModelAndView wxpay(HttpServletRequest request, 
            HttpServletResponse response) throws IOException{
		...
		LogUtil.writeLog("http://test.vk30.com/smipay/pay.jsp?appid="+orderReturn.getAppId()+"&timeStamp="+orderReturn.getTimestamp()+"&nonceStr="+orderReturn.getNonceStr()+"&package="+orderReturn.getPackageValue()+"&sign="+orderReturn.getPaySign());
		response.sendRedirect("http://test.vk30.com/smipay/pay.jsp?appid="+orderReturn.getAppId()+"&timeStamp="+orderReturn.getTimestamp()+"&nonceStr="+orderReturn.getNonceStr()+"&package="+orderReturn.getPackageValue()+"&sign="+orderReturn.getPaySign());
       return null;
    }
	
	 
 
	@RequestMapping ("/uninoPay")
            ...
            HttpServletResponse resp) throws IOException
	{
		...
		//前台页面传过来的
				...
				requestData.put("orderId","0001201604030911");             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则		
				...
				//requestData.put("frontUrl", ParamConfig.frontUrl);
