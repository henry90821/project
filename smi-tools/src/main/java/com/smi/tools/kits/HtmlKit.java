...
import com.smi.tools.http.HTMLFilter;
...
public class HtmlKit {
	...
	static {
		...
		TEXT['"'] = StrKit.HTML_QUOTE.toCharArray(); // 双引号
		TEXT['&'] = StrKit.HTML_AMP.toCharArray(); // &符
		TEXT['<'] = StrKit.HTML_LT.toCharArray(); // 小于号
		TEXT['>'] = StrKit.HTML_GT.toCharArray(); // 大于号
	}
	...
	public static String restoreEscaped(String htmlStr) {
		...
		return htmlStr
				.replace("&#39;", "'")
				.replace(StrKit.HTML_LT, "<")
				.replace(StrKit.HTML_GT, ">")
				.replace(StrKit.HTML_AMP, "&")
				.replace(StrKit.HTML_QUOTE, "\"")
				.replace(StrKit.HTML_NBSP, " ");
	}
	...
	public static String removeHtmlTag(String content, boolean withTagContent, String... tagNames) {
		...
		for (String tagName : tagNames) {
			...
			//(?i)表示其后面的表达式忽略大小写
			regex1 = StrKit.format("(?i)<{}\\s?[^>]*?/>", tagName);	
			if(withTagContent) {
				//标签及其包含内容
				regex2 = StrKit.format("(?i)(?s)<{}\\s*?[^>]*?>.*?</{}>", tagName, tagName);
			}else {
				//标签不包含内容
				regex2 = StrKit.format("(?i)<{}\\s*?[^>]*?>|</{}>", tagName, tagName);
			}
			
			content = content
					.replaceAll(regex1, StrKit.EMPTY)									//自闭标签小写
					.replaceAll(regex2, StrKit.EMPTY);									//非自闭标签小写
		}
		return content;
	}
	...
	public static String removeHtmlAttr(String content, String... attrs) {
		...
		for (String attr : attrs) {
			regex = StrKit.format("(?i)\\s*{}=([\"']).*?\\1", attr);
			content = content.replaceAll(regex, StrKit.EMPTY);
		}
		return content;
	}
	...
	public static String removeAllHtmlAttr(String content, String... tagNames) {
		String regex = null;
		for (String tagName : tagNames) {
			regex = StrKit.format("(?i)<{}[^>]*?>", tagName);
			content.replaceAll(regex, StrKit.format("<{}>", tagName));
		}
		return content;
	}
