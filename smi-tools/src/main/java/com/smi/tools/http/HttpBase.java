...
import com.smi.tools.kits.CharsetKit;
...
public abstract class HttpBase<T> {
	...
	/**编码*/
	protected String charset = CharsetKit.UTF_8;
	/**http版本*/
	...
	public T header(String name, String value, boolean isOverride) {
		if(null != name && null != value){
			...
			if(isOverride || CollectionKit.isEmpty(values)) {
	...
	public void header(Map<String, List<String>> headers) {
		...
		for (Entry<String, List<String>> entry : headers.entrySet()) {
			...
			for (String value : entry.getValue()) {
				this.header(name, StrKit.nullToEmpty(value), false);
			}
		}
	}
