...
import com.smi.tools.http.ssl.DefaultTrustManager;
import com.smi.tools.http.ssl.TrustAnyHostnameVerifier;
import com.smi.tools.kits.CollectionKit;
import com.smi.tools.kits.ObjectKit;
import com.smi.tools.kits.StrKit;
import com.smi.tools.kits.URLKit;
import com.smi.tools.lang.Validator;
...
public class HttpConnection {
	...
	public HttpConnection(String urlStr, Method method) {
		...
		this.url = URLKit.url(urlStr);
		this.method = ObjectKit.isNull(method) ? Method.GET : method;

		try {
			this.conn = HttpKit.isHttps(urlStr) ? openHttps() : openHttp();
		} catch (Exception e) {
	...
	public void setTrustManagers(TrustManager... trustManagers){
		if(CollectionKit.isNotEmpty(trustManagers)){
	...
	public HttpConnection header(Map<String, List<String>> headers) {
		if(CollectionKit.isNotEmpty(headers)) {
			String name;
			for (Entry<String, List<String>> entry : headers.entrySet()) {
				name = entry.getKey();
				for (String value : entry.getValue()) {
				    // 默认使用最新的header值进行覆盖设置
					this.header(name, StrKit.nullToEmpty(value), true);
				}
			}
		}
		return this;
	...
	public InputStream getInputStream() throws IOException {
		...
		if (StrKit.isBlank(setCookie) == false) {
	...
	public String charset() {
		return HttpKit.getCharset(conn);
	}
	...
	private HttpsURLConnection openHttps() throws IOException, NoSuchAlgorithmException, NoSuchProviderException, KeyManagementException {
		...
		final TrustManager[] trustManagers = CollectionKit.isNotEmpty(this.trustManagers) ? this.trustManagers : new TrustManager[] { new DefaultTrustManager() };
