...
public class SearchViewAction {
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response, String gc_id, String currentPage, String orderBy, String orderType, String goods_type, String goods_inventory, String keyword, String goods_transfee, String goods_cod) throws UnsupportedEncodingException {
		ModelAndView mv = new JModelAndView("lucene/search_goods_list.html", configService.getSysConfig(), this.userConfigService.getUserConfig(), 1, request, response);
		// 将关键字加入用户的搜索历史中
		if (!StringUtils.isNullOrEmpty(keyword)) {
			...
			if ("goods_current_price".equals(CommUtil.null2String(orderBy))) {
			...
			if (sort != null) {
				pList = lucene.search(keyword, CommUtil.null2Int(currentPage), 24, goods_inventory, goods_type, query_gc, goods_transfee, goods_cod, sort, null, null, null);
			}
			else {
				pList = lucene.search(keyword, CommUtil.null2Int(currentPage), 24, goods_inventory, goods_type, query_gc, goods_transfee, goods_cod, null, null, null);
			}
		...
		for (Goods compare_goods : goods_compare_list) {
			if (compare_goods != null) {
				...
				// begin dengyuqi 2015-9-17
				// 处理空指针异常，当对比栏中存在商品时，对比栏中的商品以及当前详情页面的商品均可能存在无商品分类或只存在二级分类
				if (null != compare_goods.getGc()) {
					compareGoodsGcId = compare_goods.getGc().getParent().getParent() == null ? compare_goods.getGc().getParent().getId() : compare_goods.getGc().getParent().getParent().getId();
				}
				if (compareGoodsGcId != 0 && compareGoodsGcId == CommUtil.null2Long(gc_id)) {
					compare_goods_flag = 1;
				}
				// if
				// (!compare_goods.getGc().getParent().getParent().getId().equals(CommUtil.null2Long(gc_id)))
				// {
				// compare_goods_flag = 1;
				// }
				// end
	...
	public void search_goodsclass(HttpServletRequest request, HttpServletResponse response, String keyword) {
		if (StringUtils.isNullOrEmpty(keyword)) {
			return;
		}
		...
		List<GoodsClass> objs = this.goodsClassService.query("select obj from GoodsClass obj where obj.level<:level and obj.className like :keyword", params, -1, -1);
		Map map = null;
		if (objs.size() > 0) {
			map = new HashMap();
	...
	public void search_other_goods(ModelAndView mv) {
		...
		if (this.configService.getSysConfig().isZtc_status()) {
			...
			List<Goods> all_left_ztc_goods = this.goodsService.query("select obj.id from Goods obj where obj.ztc_status =:ztc_status " + "and obj.ztc_begin_time <=:now_date and obj.ztc_gold>:ztc_gold " + "order by obj.ztc_dredge_price desc", ztc_map, -1, -1);
			left_ztc_goods = this.goodsService.query("select new Goods(id, goods_name, goods_current_price,goods_price, goods_main_photo) from Goods obj where obj.ztc_status =:ztc_status " + "and obj.ztc_begin_time <=:now_date and obj.ztc_gold>:ztc_gold " + "order by obj.ztc_dredge_price desc",
			...
			top_ztc_goods = this.goodsService.query("select new Goods(id, goods_name, goods_current_price,goods_price, goods_main_photo) from Goods obj where obj.ztc_status =:ztc_status " + "and obj.ztc_begin_time <=:now_date and obj.ztc_gold>:ztc_gold " + "order by obj.ztc_dredge_price desc",
					ztc_map2, 0, 3);
			...
			List<Goods> top_ztc_goods = this.goodsService.query("select new Goods(id, goods_name, goods_current_price,goods_price, goods_main_photo) from Goods obj where obj.store_recommend=:store_recommend and obj.goods_status=:goods_status order by obj.goods_salenum desc", params, 0, 3);
			mv.addObject("top_ztc_goods", top_ztc_goods);
			params.clear();
			params.put("store_recommend", true);
			params.put("goods_status", 0);
			// 获取所有商品数量，查询出的对象为只有id的对象，减少查询压力,查询所有直通车商品，随机显示出指定数量
			List<Goods> all_goods = this.goodsService.query("select obj.id from Goods obj where obj.store_recommend=:store_recommend and obj.goods_status=:goods_status order by obj.goods_salenum desc", params, -1, -1);
			List<Goods> left_ztc_goods = this.goodsService.query("select new Goods(id, goods_name, goods_current_price,goods_price, goods_main_photo) from Goods obj where obj.store_recommend=:store_recommend and obj.goods_status=:goods_status order by obj.goods_salenum desc", params, 3,
					all_goods.size());
			left_ztc_goods = this.goodsViewTools.randomZtcGoods2(left_ztc_goods, 8);
			mv.addObject("left_ztc_goods", left_ztc_goods);
		}
	...
	public List<GoodsClass> query_GC_second(Set<String> list_gcs) {
		...
		Map<String, Set<Long>> params = new HashMap<String, Set<Long>>();
		List<GoodsClass> gcs = new ArrayList<GoodsClass>();
		Set<Long> ids = new HashSet<Long>();
		for (String str : list_gcs) {
			// begin dengyuqi 2015-9-11 新增数组大小判断，解决商品所属分类为空时数组下表越界的bug
	...
	/**
	 * 根据店铺SEO关键字，查出关键字命中的店铺(未正常营业的商店不会被返回)
	 * 
	 * @param keyword
	 * @return
	 */
	public List<Store> search_stores_seo(String keyword) {
		...
		List<Store> stores = this.storeService.query("select obj from Store obj where obj.store_status = 15 and (obj.store_seo_keywords =:keyword1 or obj.store_seo_keywords like:keyword2 or obj.store_seo_keywords like:keyword3 or obj.store_seo_keywords like:keyword4)", params, 0, 3);
	 ...
	 */
	public Cookie search_history_cookie(HttpServletRequest request, String keyword) throws UnsupportedEncodingException {
		String str = "";
		Cookie[] cookies = request.getCookies();
		Cookie search_cookie = null;
		for (Cookie cookie : cookies) {
			if ("search_history".equals(cookie.getName())) {
				String str_temp = URLDecoder.decode(cookie.getValue(), "UTF-8");
				for (String s : str_temp.split(",")) {
					if (!s.equals(keyword) && !"".equals(str)) {
						str = str + "," + s;
					}
