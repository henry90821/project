...
public class LuceneVoTools {
	...
	public LuceneVo updateGoodsIndex(Goods goods) {
		...
		vo.setVo_well_evaluate(CommUtil.div(new Integer(goods.getWell_evaluate()), new Integer(goods.getEvaluate_count())));
		vo.setVo_goods_inventory(goods.getGoods_inventory());
		vo.setVo_goods_type(goods.getGoods_type());
		vo.setSeckill_buy(goods.getSeckill_buy());
		vo.setVo_goods_config(goods.getGoodsConfig()==null?null:goods.getGoodsConfig().getConfigCode());
		if (goods.getGoods_brand() != null) {
			vo.setVo_goods_brandname(goods.getGoods_brand().getName());
		}
		if (goods.getGoods_main_photo() != null) {
			vo.setVo_main_photo_url(goods.getGoods_main_photo().getPath() + "/" + goods.getGoods_main_photo().getName() + "_middle." + goods.getGoods_main_photo().getExt());
		}
		...
		vo.setVo_goods_evas(goods.getEvaluate_count());

		// begin dengyuqi 2015-9-10 加入为空处理，解决当商品所属分类为空时报nullexception的问题
		if (null != goods.getGc() && goods.getGc().getLevel() == 2) {
			vo.setVo_goods_class(CommUtil.null2String(goods.getGc().getParent().getId()) + "_" + CommUtil.null2String(goods.getGc().getId()));
		}
		else {
			vo.setVo_goods_class(CommUtil.null2String(null == goods.getGc() ? null : goods.getGc().getId()) + "_");
		}
		// end
	...
	public LuceneVo updateLifeGoodsIndex(GroupLifeGoods goods) {
		...
		vo.setVo_goods_inventory(goods.getGroup_count());
		...
		String rate = this.groupViewTools.getRate(CommUtil.null2Double(goods.getGroup_price()), CommUtil.null2Double(goods.getCost_price())).toString();
		vo.setVo_rate(rate);
	...
	public LuceneVo updateGroupGoodsIndex(GroupGoods goods) {
		...
		vo.setVo_goods_inventory(goods.getGg_count());
