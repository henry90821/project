...
public class SellerApplyAction {
	...
	public ModelAndView seller_apply(HttpServletRequest request, HttpServletResponse response, String op, String step) {
		...
		if (user.getStore_apply_step() > 7) { // 申请分为7步，走完第7步流程进入审核状态，就不允许继续修改资料了
			...
			if (step != null && !"".equals(CommUtil.null2String(step))) { // 如果step参数不为空，更新申请步骤
			...
			if ("back".equals(CommUtil.null2String(op)) && (user.getStore_apply_step() - CommUtil.null2Int(step) >= 1)) { // 如果执行的返回操作
	...
	public String seller_apply_step2_save(HttpServletRequest request, HttpServletResponse response, String trueName,
		...
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		if (!"".equals(CommUtil.null2String(trueName)) && !"".equals(CommUtil.null2String(telephone))
				&& !"".equals(CommUtil.null2String(email))) {
	...
	public String seller_apply_step4_save(HttpServletRequest request, HttpServletResponse response, String bid2, String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
		WebForm wf = new WebForm();
	...
	@SecurityMapping(title = "商家入驻申请", value = "/seller_account_check.htm*", rtype = "buyer", rname = "商家入驻", rcode = "seller_apply", rgroup = "商家入驻")
	@RequestMapping("/seller_account_check.htm")
	public void seller_account_check(HttpServletRequest request, HttpServletResponse response, String account) {
		...
		try {
			...
			writer.print(!this.userService.isSellerAccountExisted(accountValid));
		} catch (IOException e) {
	...
	public String seller_apply_step5_save(HttpServletRequest request, HttpServletResponse response, String aid2,
			String step, String storeGrades, String gc_main_id, String validity, String sellerLoginAccount) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		sellerLoginAccount = CommUtil.clearContent(sellerLoginAccount);
		if(userService.isSellerAccountExisted(sellerLoginAccount)) {
			logger.error("店铺登录账号(" + sellerLoginAccount + ")已存在，请使用其它店铺登录账号!");
			return "redirect:seller_apply.htm";
		}
	 ...
	 * @return
	 */
	@SecurityMapping(title = "入驻店铺等级保存", value = "/seller_apply_step6_save.htm*", rtype = "buyer", rname = "商家入驻", rcode = "seller_apply", rgroup = "商家入驻")
	@RequestMapping("/seller_apply_step6_save.htm")
	public String seller_apply_step6_save(HttpServletRequest request, HttpServletResponse response, String goodsClass_main,
			String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user.setStore_apply_step(CommUtil.null2Int(step));
		this.userService.update(user);
	...
	public ModelAndView seller_store_query(HttpServletRequest request, HttpServletResponse response) {
		...
		if (store.getStore_status() == 15 || store.getStore_status() == 20 || store.getStore_status() == 25) {
			mv = new JModelAndView("seller_apply/seller_apply_wait3.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
			mv.addObject("loginAccount", user.getSellerLoginAccount());
		}
		mv.addObject("store", store);
		return mv;
	}
	...
	public ModelAndView seller_store_rewrite(HttpServletRequest request, HttpServletResponse response, String step) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		if ("3".equals(step)) { // 重新填写公司信息
			user.setStore_apply_step(3);
		}
		if ("5".equals(step)) { // 重新填写店铺信息
	...
	public void seller_apply_image_save(HttpServletRequest request, HttpServletResponse response, String mark, String uid,
			String jsessionid) {
		// HttpSessionContext SessCon=
		// request.getSession(true).getSessionContext();
		...
		if ("idCard".equals(mark)) { // 保存身份证电子版
			...
			try {
				map = CommUtil.saveFileToServer(request, mark + "_img_a", saveFilePathName, "", null);
				if (map.get("fileName") != "") {
					...
					photo.setPath(this.FTPTools.userUpload(photo.getName(), "/store/" + store.getId(),
	...
	public void seller_apply_image_del(HttpServletRequest request, HttpServletResponse response, String mark, String uid) {
		User user = this.userService.getObjById(CommUtil.null2Long(uid));
		Store store = user.getStore();
		response.setContentType("text/plain");
		response.setHeader("Cache-Control", "no-cache");
		...
		if ("idCard".equals(mark)) { // 删除身份证电子版
			...
			try {
				...
				if (ret) {
					CommUtil.del_acc(request, img);
					this.FTPTools.userDeleteFtpImg(img, uid);
				}
				json.put("result", ret);
				writer = response.getWriter();
	...
	public ModelAndView seller_goodsclass_ajax(HttpServletRequest request, HttpServletResponse response, String cid,
			String uid) {
		ModelAndView mv = new JModelAndView("seller_apply/seller_store_ajax_goodsClass.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		User user = this.userService.getObjById(CommUtil.null2Long(uid));
		Store user_store = user.getStore();
	...
	public ModelAndView seller_goodsclass_save(HttpServletRequest request, HttpServletResponse response, String gc_ids,
		...
		Map map_temp = this.storeTools.query_MainGc_Map(CommUtil.null2String(parent.getId()), store.getGc_detail_info());
		...
		list.add(map);
		if(list.size() == 0) {//store的storeGC_detail_info字段要么为null要么为非空的json格式的数组，不能为"[]"，所以当list为空时要返回null，而不是"[]"
			store.setGc_detail_info(null);
		} else {
	...
	public ModelAndView seller_goodsclass_del(HttpServletRequest request, HttpServletResponse response, String gc_id) {
		...
		Map map_temp = this.storeTools.query_MainGc_Map(CommUtil.null2String(gc_main.getId()), store.getGc_detail_info());
		...
		for (GoodsClass gc : gc_mains) {
			if (gc.getId() == gc_main.getId()) { // 删除对应类目
				...
				Map map_main = this.storeTools.query_MainGc_Map(CommUtil.null2String(gc.getId()), store.getGc_detail_info());
				list.add(map_main);
			}
		}
		if(list.size() == 0) {//store的storeGC_detail_info字段要么为null要么为非空的json格式的数组，不能为"[]"，所以当list为空时要返回null，而不是"[]"
