 ...
 * 
 * <p>
 * Description:动态url处理控制器，该控制器用来处理相关动态url，比如商品url，前端页面中仅仅有商品id， 无法判断商品url是通过二级域名访问还是顶级域名访问，此时就需要使用该控制的一个方法完成url跳转
 * </p>
 * 
 * <p>
...
public class DynamicUrlViewAction {
	...
	public void goods_dynamic(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		if (this.configService.getSysConfig().isSecond_domain_open()
				&& !StringUtils.isNullOrEmpty(obj.getGoods_store().getStore_second_domain())) {
			goods_url = "http://" + obj.getGoods_store().getStore_second_domain() + "." + CommUtil.generic_domain(request)
					+ "/goods_" + id + ".htm";
		}
		try {
