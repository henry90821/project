...
public class ConsultViewTools {
	...
	public List<Consult> queryByType(String type, String goods_id) {
		...
		if (!"".equals(CommUtil.null2String(type))) {
			...
			list = this.consultService.query(
					"select obj from Consult obj where obj.consult_type=:consult_type and obj.goods_id=:goods_id", params,
					-1, -1);
		} else {
			Map params = new HashMap();
			params.put("goods_id", CommUtil.null2Long(goods_id));
			list = this.consultService.query("select obj from Consult obj where obj.goods_id=:goods_id", params, -1, -1);
		}
		return list;
	}
