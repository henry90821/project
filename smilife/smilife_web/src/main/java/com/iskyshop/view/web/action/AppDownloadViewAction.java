...
public class AppDownloadViewAction {
	...
	public void app_download(HttpServletRequest request, HttpServletResponse response) {
		logger.debug(request.getHeader("User-Agent").toLowerCase());
	...
	public void app_seller_download(HttpServletRequest request, HttpServletResponse response) {
		String user_agent = request.getHeader("User-Agent").toLowerCase();
		String url = CommUtil.getURL(request);
		// String ios_reg =
		// ".+?\\(iphone; cpu \\w+ os [1-9]\\d*_\\d+_\\d+ \\w+ mac os x\\).+";
