...
public class GroupViewTools {
	...
	public List<GroupGoods> query_goods(String group_id, int count) {
		...
		list = this.groupGoodsService.query(
				"select obj from GroupGoods obj where obj.group.id=:group_id order by obj.addTime desc", params, 0, count);
		return list;
	}

	public static Double getRate(Double group_price, Double cost_price) {
		double ret = 0.0;
		if (!"".equals(CommUtil.null2String(group_price)) && !"".equals(CommUtil.null2String(cost_price))) {
