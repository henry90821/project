...
public class SystemLogAdvice {
	...
	public void admin_op_log(JoinPoint joinPoint, SecurityMapping annotation, HttpServletRequest request) throws Exception {
		if (Globals.SAVE_LOG) {
	...
	public void admin_app_log(JoinPoint joinPoint, SecurityMapping annotation, HttpServletRequest request) throws Exception {
		if (Globals.SAVE_LOG) {
			saveLog(joinPoint, annotation, request);
		}
	}
	...
	public void admin_cms_log(JoinPoint joinPoint, SecurityMapping annotation, HttpServletRequest request) throws Exception {
		if (Globals.SAVE_LOG) {
			saveLog(joinPoint, annotation, request);
		}
	}
	...
	public void admin_sns_log(JoinPoint joinPoint, SecurityMapping annotation, HttpServletRequest request) throws Exception {
		if (Globals.SAVE_LOG) {
			saveLog(joinPoint, annotation, request);
		}
	}

	private void saveLog(JoinPoint joinPoint, SecurityMapping annotation, HttpServletRequest request) throws Exception {
		if (SecurityUserHolder.getCurrentUser() != null) {
			User user = this.userSerivce.getObjById(SecurityUserHolder.getCurrentUser().getId());
			// 获取操作内容
			...
			if (SecurityUserHolder.getCurrentUser().getTrueName() != null) {
				userName = userName + "（" + SecurityUserHolder.getCurrentUser().getTrueName() + "）";
			}
			String description = userName + "于" + CommUtil.formatTime("yyyy-MM-dd HH:mm:ss", new Date());
			if (annotation.value().indexOf("index") > 0) {
				...
				if (id != null || mulitId != null || annotation.value().indexOf("save") >= 0
						|| annotation.value().indexOf("edit") >= 0 || annotation.value().indexOf("update") >= 0) {
					String option1 = "执行";
					String option2 = "操作";
	...
	public void exceptionLog(HttpServletRequest request, Throwable exception) {
		if (Globals.SAVE_LOG) {
			if (SecurityUserHolder.getCurrentUser() != null) {
				...
				log.setAddTime(new Date());
				log.setUser_name(user.getUserName());
				log.setContent(log.getAddTime() + "执行" + request.getRequestURI() == null ? ""
