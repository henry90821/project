...
public class KuaidiCallback {
	...
	public void kuaidi100_callback(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		try {
			...
			// 处理快递结果
			Long order_id = CommUtil.null2Long(request.getParameter("order_id"));
			int order_type = CommUtil.null2Int(request.getParameter("order_type"));
			ExpressInfo obj = this.expressInfoService.getObjByPropertyWithType("order_id", order_id, order_type);
			// logger.debug(param);
			String sign = CommUtil.null2String(request.getParameter("sign")); // 返回的签名值
			String salt = Md5Encrypt.md5(CommUtil.null2String(order_id)).toLowerCase();
			String sign1 = Md5Encrypt.md5(param + salt).toUpperCase(); // 根据快递100的规则计算的签名值，2者一致才认为是快递100返回数据，避免恶意数据攻击
			logger.debug("返回签名为：" + sign + ",计算的签名为：" + sign1);
			if (sign.equals(sign1)) {
				if (obj == null) {
					...
					logger.debug(Json.toJson(result.getData(), JsonFormat.compact()));
					obj.setOrder_express_info(Json.toJson(result.getData(), JsonFormat.compact()));
