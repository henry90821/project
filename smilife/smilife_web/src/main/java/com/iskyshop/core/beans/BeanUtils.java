...
public abstract class BeanUtils {
	...
	/**
	 * Find a method with the given method name and minimal parameters (best case: none), declared on the given class or one
	 * of its superclasses. Prefers public methods, but will return a protected, package access, or private method too.
	 * <p>
	 * Checks <code>Class.getMethods</code> first, falling back to <code>findDeclaredMethodWithMinimalParameters</code>. This
	 * allows to find public methods without issues even in environments with restricted Java security settings.
	 * 
	 * @param clazz
	 *            the class to check
	 * @param methodName
	 ...
	 *             if methods of the given name were found but could not be resolved to a unique method with minimal
	 *             parameters
	...
	/**
	 * Find a method with the given method name and minimal parameters (best case: none), declared on the given class or one
	 * of its superclasses. Will return a public, protected, package access, or private method.
	 * <p>
	 ...
	 * <p>
	 * When not supplying an argument list (<code>methodName</code>) the method whose name matches and has the least number
	 * of parameters will be returned. When supplying an argument type list, only the method whose name and argument types
	 * match will be returned.
	 * <p>
	 * Note then that <code>methodName</code> and <code>methodName()</code> are <strong>not</strong> resolved in the same
	 * way. The signature <code>methodName</code> means the method called <code>methodName</code> with the least number of
	 * arguments, whereas <code>methodName()</code> means the method called <code>methodName</code> with exactly 0 arguments.
	 * <p>
	...
	public static Method resolveSignature(String signature, Class clazz) {
		...
		if (firstParen > -1 && lastParen == -1) {
			...
			String[] parameterTypeNames = StringUtils.commaDelimitedListToStringArray(signature.substring(firstParen + 1,
					lastParen));
	...
	/**
	 * Find a JavaBeans <code>PropertyDescriptor</code> for the given method, with the method either being the read method or
	 * the write method for that bean property.
	 * 
	 * @param method
	...
	public static String canonicalPropertyName(String propertyName) {
		...
		while (searchIndex != -1) {
			...
			searchIndex = -1;
			if (keyStart != -1) {
				int keyEnd = buf.toString().indexOf(PropertyAccessor.PROPERTY_KEY_SUFFIX,
	...
	/**
	 * Copy the property values of the given source bean into the given target bean, only setting properties defined in the
	 * given "editable" class (or interface).
	 * <p>
	 * Note: The source and target classes do not have to match or even be derived from each other, as long as the properties
	 * match. Any bean properties that the source bean exposes but the target bean does not will silently be ignored.
	 * <p>
