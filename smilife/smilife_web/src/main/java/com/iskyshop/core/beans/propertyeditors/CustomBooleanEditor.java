...
public class CustomBooleanEditor extends PropertyEditorSupport {
	 ...
	 * value. Else, an IllegalArgumentException gets thrown in that case.
	 * 
	 * @param allowEmpty
	 ...
	 * 
	 * @param trueString
	 *            the String value that represents true: for example, "true" (VALUE_TRUE), "on" (VALUE_ON), "yes" (VALUE_YES)
	 *            or some custom value
	 * @param falseString
	 *            the String value that represents false: for example, "false" (VALUE_FALSE), "off" (VALUE_OFF), "no"
	 *            (VALUE_NO) or some custom value
	 * @param allowEmpty
	 *            if empty strings should be allowed
	 * @see #VALUE_TRUE
	 * @see #VALUE_FALSE
	...
	public void setAsText(String text) throws IllegalArgumentException {
		if (this.allowEmpty && !StringUtils.hasText(text)) {
			...
			setValue(Boolean.FALSE);
				...
				&& (text.equalsIgnoreCase(VALUE_TRUE) || text.equalsIgnoreCase(VALUE_ON) || text.equalsIgnoreCase(VALUE_YES) || text
						.equals(VALUE_1))) {
			setValue(Boolean.TRUE);
