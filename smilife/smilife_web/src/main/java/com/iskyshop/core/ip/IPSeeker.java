...
public class IPSeeker {
	...
	// 保存的文件夹
	private String INSTALL_DIR = CommUtil.getServerRealPathFromSystemProp() + "resources" + File.separator + "data";
	...
	public IPSeeker(String fileName, String dir) {
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(INSTALL_DIR + File.separator + IP_FILE, "r");
					...
					if (files[i].isFile()) {
						if (files[i].getName().toLowerCase().equals(filename)) {
							try {
								...
								LogFactory.log("IP地址信息文件没有找到，IP显示功能将无法使用", Level.ALL, e1);
								ipFile = null;
							}
							break;
						}
