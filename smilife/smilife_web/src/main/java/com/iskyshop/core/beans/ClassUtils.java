...
/**
 * Miscellaneous class utility methods. Mainly for internal use within the framework; consider Jakarta's Commons Lang for a
 * more comprehensive suite of utilities.
 * 
 * @author Keith Donald
 * @author Rob Harrop
...
public abstract class ClassUtils {
	 ...
	 * int.class.
	 */
	private static Map primitiveWrapperTypeMap = new HashMap(8);

	/**
	 * Map with primitive type name as key and corresponding primitive type as value, for example: "int" -> "int.class".
	 */
	private static Map primitiveTypeNameMap = new HashMap(8);
	...
	/**
	 * Return a default ClassLoader to use (never <code>null</code>). Returns the thread context ClassLoader, if available.
	...
	/**
	 * Return whether the {@link Class} identified by the supplied name is present and can be loaded. Will return
	 ...
	 * <p>
	 * Always uses the default class loader: that is, preferably the thread context class loader, or the ClassLoader that
	 * loaded the ClassUtils class as fallback.
	 * 
	 * @param name
	 ...
	 * @return the primitive class, or <code>null</code> if the name does not denote a primitive class
	 */
	...
	/**
	 * Return the qualified name of the given class: usually simply the class name, but component type class name + "[]" for
	 * arrays.
	 * 
	 * @param clazz
	 *            the class
	 ...
	 * <code>NoSuchMethodException</code> to "false".
	 * 
	 * @param clazz
	...
	/**
	 * Does the given class and/or its superclasses at least have one or more methods (with any argument types)? Includes
	 * non-public methods.
	 * 
	 * @param clazz
	 *            the clazz to check
	...
	/**
	 * Check if the given class represents a primitive wrapper, i.e. Boolean, Byte, Character, Short, Integer, Long, Float,
	 * or Double.
	 */
	...
	/**
	 * Check if the given class represents a primitive (i.e. boolean, byte, char, short, int, long, float, or double) or a
	 * primitive wrapper (i.e. Boolean, Byte, Character, Short, Integer, Long, Float, or Double).
	 */
	...
	/**
	 * Check if the given class represents an array of primitive wrappers, i.e. Boolean, Byte, Character, Short, Integer,
	...
	/**
	 * Determine if the given target type is assignable from the given value type, assuming setting by reflection. Considers
	...
	/**
	 * Determine if the given type is assignable from the given value, assuming setting by reflection. Considers primitive
	 * wrapper classes as assignable to the corresponding primitive types.
	 * 
	 * @param type
	 *            the target type
	 * @param value
	 ...
	 * <code>Class.getResource</code> by prepending a slash ('/') to the return value. Built by taking the package of the
	 * specified class file, converting all dots ('.') to slashes ('/'), adding a trailing slash if necesssary, and
	 * concatenating the specified resource name to this. <br/>
	 * As such, this function may be used to build a path suitable for loading a resource file that is in the same package as
	 * a class file, although {@link org.springframework.core.io.ClassPathResource} is usually even more convenient.
	 * 
	 * @param clazz
	...
	/**
	 * Given an input class object, return a string which consists of the class's package name as a pathname, i.e., all dots
	 * ('.') are replaced by slashes ('/'). Neither a leading nor trailing slash is added. The result could be concatenated
	 * with a slash and the name of a resource, and fed directly to ClassLoader.getResource(). For it to be fed to
	 * Class.getResource, a leading slash would also have to be prepended to the return value.
	 * 
	 * @param clazz
	 *            the input class. A null value or the default (empty) package will result in an empty string ("") being
	 *            returned.
