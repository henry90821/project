...
public class PropertyEditorRegistry {
	...
	/**
	 * Determine the property type for the given property path. Called by <code>findCustomEditor</code> if no required type
	 * has been specified, to be able to find a type-specific editor even if just given a property path.
	 * <p>
	 * Default implementation always returns <code>null</code>. BeanWrapperImpl overrides this with the standard
	 * <code>getPropertyType</code> method as defined by the BeanWrapper interface.
	 * 
	 * @param propertyPath
	...
	/**
	 * Get custom editor for the given type. If no direct match found, try custom editor for superclass (which will in any
	 * case be able to render a value as String via <code>getAsText</code>).
	 * 
	...
	/**
	 * Guess the property type of the specified property from the registered custom editors (provided that they were
	 ...
	 *            the nested property path of the target registry, if any. If this is non-null, only editors registered for a
	 *            path below this nested property will be copied.
	 */
	...
	private static class CustomEditorHolder {
		...
		private PropertyEditor getPropertyEditor(Class requiredType) {
			...
			if (this.registeredType == null
					|| (requiredType != null && (ClassUtils.isAssignable(this.registeredType, requiredType) || ClassUtils
							.isAssignable(requiredType, this.registeredType)))
					|| (requiredType == null && (!Collection.class.isAssignableFrom(this.registeredType) && !this.registeredType
							.isArray()))) {
