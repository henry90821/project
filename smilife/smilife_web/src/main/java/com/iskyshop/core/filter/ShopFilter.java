 ...
 * 
 * <p>
 * Description:系統基础过滤器，主要用在系统第一次运行时进入安装界面，引导用户安装系统，导入系统基础数据信息， 对于资源信息如css等该过滤器将不进行首次安装拦截
 * </p>
 * 
 * <p>
...
public class ShopFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		String redirect_url = "";
		String path = CommUtil.getServerRealPathFromRequest(request) + "install.lock";
		File file = new File(path);
		if (file.exists()) {
			if (!config.isWebsiteState()) {
				if (this.init_url(url)) {
					if (url.indexOf("/admin") < 0 && url.indexOf("/install.htm") <= 0 && url.indexOf("/close.htm") < 0
							&& url.indexOf("/verify.htm") < 0 && url.indexOf("login_success.htm") < 0) {
						redirect = true;
						redirect_url = CommUtil.getURL(request) + "/close.htm";
					}
					if (url.indexOf("/mobile") >= 0) {
						redirect = true;
						redirect_url = CommUtil.getURL(request) + "/mobile/close.htm";
					}
				}
			} else {
	...
	private boolean init_url(String url) {
		...
		String[] extend_list = new String[] { "css", "jpg", "jpeg", "png", "gif", "bmp", "tbi", "js" };
		boolean flag = true;
		for (String temp : extend_list) {
			if (prifix.equals(temp)) {
				flag = false;
			}
		}
