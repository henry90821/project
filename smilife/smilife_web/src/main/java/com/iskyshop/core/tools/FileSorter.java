...
public class FileSorter implements Comparator<File> {
	private static Logger logger = Logger.getLogger(FileSorter.class);
	...
	/*
	 * public static final int TYPE_NAME_DOWN = 5; public static final int TYPE_NAME_UP = 6;
	 */
	/** 按文件名 */
	...
	private static void printFileArray(File[] list) {
		logger.info("文件大小\t\t文件修改日期\t\t文件类型\t\t文件名称");
		for (File f : list) {
			logger.info(f.length() + "\t\t" + new Date(f.lastModified()).toString() + "\t\t"
