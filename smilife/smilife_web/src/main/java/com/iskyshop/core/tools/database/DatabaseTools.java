	...
	public boolean createSqlScript(HttpServletRequest request, String path, String name, String size, String tables)
		...
		try {
			...
			for (String table : backup_list) {
				...
				List<String> list = publicMethod.getAllColumns(strBuilder.toString());
				for (String line : list) {
					...
					request.getSession(true).setAttribute("db_mode", "backup");
					request.getSession(true).setAttribute("db_bound", count);
		...
		try {
			...
			for (String table : backup_list) {
				if (!"".equals(CommUtil.null2String(table))) {
					// 生成insert语句
	...
	public List<String> getAllDatas(String tableName) throws Exception {
		...
		try {
			...
			while (rs.next()) {
				...
				insert_sql.append(
						"INSERT INTO " + tableName + " (" + typeStr.substring(0, typeStr.length() - 1) + ") VALUES (");
				...
				for (TableColumn dbColumn : columnList) {
					...
					if (null == rs.getString(columnsFile)) {
						...
						vector.add(Integer.valueOf(rs.getString(columnsFile)).intValue());
							...
							&& 0 == Integer.valueOf(rs.getString(columnsFile)).intValue()) {
						vector.add("\'" + "\'");
					} else if ("longblob".equals(columnsType) || "blob".equals(columnsType) || "tinyblob".equals(columnsType)
							|| "mediumblob".equals(columnsType)) {
						vector.add("0x" + rs.getString(columnsFile));
						// 处理
					} else if ("text".equals(columnsType) || "longtext".equals(columnsType) || "tinytext".equals(columnsType)
