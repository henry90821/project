...
public interface PropertyAccessor {
	...
	/**
	 * Determine the property type for a particular property, either checking the property descriptor or checking the value
	 * in case of an indexed or mapped element.
	 * 
	 * @param propertyName
	 *            property to check status for
	 * @return the property type for the particular property, or <code>null</code> if not determinable
	 */
	 ...
	 * <p>
	 * Bulk updates from PropertyValues are more powerful: This method is provided for convenience. Behaviour will be
	 * identical to that of the setPropertyValues(PropertyValues) method.
	 * 
	 * @param map
	 *            Map to take properties from. Contains property value objects, keyed by property name
	 * @throws FatalBeanException
	 *             if there is no such property, if the property isn't writable, or if the property setter throws an
	 *             exception.
	 */
	 ...
	 * <p>
	 * Note that performing a bulk update differs from performing a single update, in that an implementation of this class
	 * will continue to update properties if a <b>recoverable</b> error (such as a type mismatch, but <b>not</b> an invalid
	 * field name or the like) is encountered, throwing a PropertyAccessExceptionsException containing all the individual
	 * errors. This exception can be examined later to see all binding errors. Properties that were successfully updated stay
	 * changed.
	 * <p>
