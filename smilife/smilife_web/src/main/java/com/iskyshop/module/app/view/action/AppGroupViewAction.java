...
public class AppGroupViewAction {
	...
	public void group_index(HttpServletRequest request, HttpServletResponse response, String orderBy, String orderType,
			String gc_id, String gpr_id, String ga_id, String type, String begincount, String selectcount) {
		...
		if ("".equals(type) || "goods".equals(type) || type == null) {
			...
			if (groups.size() > 0) {
				...
				if (gpr != null) {
					params.put("begin_price", BigDecimal.valueOf(gpr.getGpr_begin()));
					params.put("end_price", BigDecimal.valueOf(gpr.getGpr_end()));
				...
				List<GroupGoods> groupGoods = this.groupGoodsService.query(sb.toString(), params,
						CommUtil.null2Int(begincount), CommUtil.null2Int(selectcount));
				for (GroupGoods gg : groupGoods) {
					Map map = new HashMap();
					map.put("gg_img", gg.getGg_img().getPath() + "/" + gg.getGg_img().getName());
		...
		if ("life".equals(type)) {
			...
			if (groups.size() > 0) {
				...
				List<GroupLifeGoods> groupGoods = this.groupLifeGoodsService.query(sb.toString(), params,
						CommUtil.null2Int(begincount), CommUtil.null2Int(selectcount));
				for (GroupLifeGoods gg : groupGoods) {
					Map map = new HashMap();
					map.put("gg_img", gg.getGroup_acc().getPath() + "/" + gg.getGroup_acc().getName());
					map.put("gg_name", gg.getGg_name());
					map.put("gg_price", CommUtil.null2Double(gg.getGroup_price()));
	...
	public void group_are(HttpServletRequest request, HttpServletResponse response) {
		List list = new ArrayList();
	...
	public void group_class(HttpServletRequest request, HttpServletResponse response, String type) {
		Map json = new HashMap();
		...
		if ("".equals(type) || "goods".equals(type) || type == null) {
			...
			if (groups.size() > 0) {
				gcs = this.groupClassService.query(
						"select obj from GroupClass obj where obj.gc_type=0 and obj.parent.id is null order by obj.gc_sequence asc",
						null, -1, -1);
				gprs = this.groupPriceRangeService.query("select obj from GroupPriceRange obj order by obj.gpr_begin asc",
						null, -1, -1);
			}
		}
	...
	public void group_goods(HttpServletRequest request, HttpServletResponse response, String id) {
		GroupGoods groupGoods = this.groupGoodsService.getObjById(CommUtil.null2Long(id));
		Map json = new HashMap();
		json.put("gg_img", groupGoods.getGg_img().getPath() + "/" + groupGoods.getGg_img().getName());
		...
		json.put("gg_endTime", CommUtil.formatShortDate(groupGoods.getEndTime()));
		...
		Goods goods = groupGoods.getGg_goods();
		json.put("goods_well_evaluate", CommUtil.mul(CommUtil.div(new Integer(goods.getWell_evaluate()),  new Integer(goods.getEvaluate_count())), 100) + "%"); // 好评率
		json.put("goods_middle_evaluate", CommUtil.mul(CommUtil.div(new Integer(goods.getMiddle_evaluate()),  new Integer(goods.getEvaluate_count())), 100) + "%"); // 中评率
		json.put("goods_bad_evaluate", CommUtil.mul(CommUtil.div(new Integer(goods.getBad_evaluate()),  new Integer(goods.getEvaluate_count())), 100) + "%"); // 差评率
		json.put("evaluate_count", goods.getEvaluate_count()); // 总评论数
	...
	public ModelAndView group_goods_introduce(HttpServletRequest request, HttpServletResponse response, String id) {
	...
	public void group_life(HttpServletRequest request, HttpServletResponse response, String id) {
		GroupLifeGoods groupLifeGoods = this.groupLifeGoodsService.getObjById(CommUtil.null2Long(id));
		Map json = new HashMap();
		json.put("gg_img", groupLifeGoods.getGroup_acc().getPath() + "/" + groupLifeGoods.getGroup_acc().getName());
		json.put("gg_name", groupLifeGoods.getGg_name());
		json.put("gg_price", CommUtil.null2Double(groupLifeGoods.getGroup_price()));
		json.put("gg_selled_count", groupLifeGoods.getSelled_count());

		json.put("gg_rebate", CommUtil.null2Double(groupLifeGoods.getGg_rebate()));
		json.put("gg_endTime", CommUtil.formatShortDate(groupLifeGoods.getEndTime()));
		json.put("gg_selled_count", groupLifeGoods.getSelled_count());
	...
	public ModelAndView group_life_introduce(HttpServletRequest request, HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("app/simple_goods_introduce.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		GroupLifeGoods groupLifeGoods = this.groupLifeGoodsService.getObjById(CommUtil.null2Long(id));
	...
	public void group_life_order_save(HttpServletRequest request, HttpServletResponse response, String user_id, String token,
				...
				if (user.getApp_login_token().equals(token.toLowerCase())) {
					...
					if (!StringUtils.isNullOrEmpty(group_id)) {
						...
						json.put("goods_mainphoto_path",
								group.getGroup_acc().getPath() + "/" + group.getGroup_acc().getName());
						...
						orderForm.setOrder_id("life" + user.getId() + CommUtil.formatTime("yyyyMMddHHmmss", new Date()));
	...
	private Payment get_payment(String mark) {
		...
		List<Payment> payments = this.paymentService
				.query("select obj from Payment obj where obj.mark in(:marks) and obj.install=:install", params, -1, -1);
		if (payments.size() > 0) {
