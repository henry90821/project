...
public class SnsBuyerAction {
	...
	public ModelAndView my_sns_index(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_index.html", configService.getSysConfig(),
		...
		List<SnsAttention> tempSnss = this.snsAttentionService.query(
				"select obj from SnsAttention obj where obj.fromUser.id=:fromUser order by obj.addTime desc", params, 0, 10);
		...
		for (SnsAttention sns : tempSnss) {
			...
			if (sns.getToUser().getPhoto() != null) {
				map.put("user_photo", sns.getToUser().getPhoto().getPath() + "/" + sns.getToUser().getPhoto().getName());
			}
		...
		tempSnss = this.snsAttentionService.query(
				"select obj from SnsAttention obj where obj.toUser.id=:toUser order by obj.addTime desc", params, 0, 10);
		...
		for (SnsAttention sns : tempSnss) {
			...
			if (sns.getFromUser().getPhoto() != null) {
				map.put("user_photo", sns.getFromUser().getPhoto().getPath() + "/" + sns.getFromUser().getPhoto().getName());
			}
		...
		List<Evaluate> evas = this.evaluateService.query(
				"select obj from Evaluate obj where obj.evaluate_user.id = :user_id and obj.evaluate_status=0 and  (obj.evaluate_photos is null or obj.evaluate_photos='') order by obj.addTime desc",
				params, 0, 2);
		mv.addObject("evas", evas);
		params.put("user_id", user.getId());
		List<Evaluate> evaPhotos = this.evaluateService.query(
				"select obj from Evaluate obj where obj.evaluate_user.id = :user_id and obj.evaluate_status=0 and  (obj.evaluate_photos is not null and obj.evaluate_photos!='') order by obj.addTime desc",
				params, 0, 2);
		mv.addObject("evaPhotos", evaPhotos);
		// 我的圈子和帖子信息
		params.clear();
		params.put("user_id", user.getId());
		List<CircleInvitation> invitations = this.invitationService.query(
				"select obj from CircleInvitation obj where obj.user_id=:user_id order by addTime desc", params, 0, 1);
		if (invitations.size() > 0) {
			mv.addObject("invi", invitations.get(0));
		}
		if (!StringUtils.isNullOrEmpty(user.getCircle_attention_info())) {
			Set<Long> ids = new HashSet<Long>();
			List<Map> maps = Json.fromJson(List.class, user.getCircle_attention_info());
			for (Map map : maps) {
			...
			if (!ids.isEmpty()) {
				params.clear();
				params.put("ids", ids);
				List<Circle> circles = this.circleService.query("select obj from Circle obj where obj.id in(:ids)", params,
						0, 3);
		...
		params.clear();
		params.put("user_id", user.getId());
		List<UserDynamic> userDynamics = this.dynamicService.query(
				"select obj from UserDynamic obj where obj.user_id =:user_id order by obj.addTime desc", params, 0, 1);
	...
	public ModelAndView my_sns_head(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_head.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public ModelAndView my_sns_nav(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_nav.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void sns_lock_on(HttpServletRequest request, HttpServletResponse response) {
		int ret = 1;
	...
	public void sns_lock_off(HttpServletRequest request, HttpServletResponse response) {
		int ret = 1;
		if (SecurityUserHolder.getCurrentUser() != null) {
			User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public ModelAndView my_sns_share(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_share.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		String param = "";
		UserShareQueryObject qo = new UserShareQueryObject(currentPage, mv, "addTime", "desc");
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/buyer/my_sns_share.htm", "", param, pList, mv);
	...
	public void my_sns_share_del(HttpServletRequest request, HttpServletResponse response, String share_id) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		boolean ret = false;
	...
	public ModelAndView my_sns_fav(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_fav.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		Map params = new HashMap();
		params.put("user_id", user.getId());
		List<Favorite> favorites = this.favoriteService.query(
				"select obj from Favorite obj where obj.user_id = :user_id and obj.type=0 order by obj.addTime desc", params,
				0, 10);
	...
	public ModelAndView sns_ajax_favs(HttpServletRequest request, HttpServletResponse response, String size) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/sns_ajax_favs.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
	...
	public ModelAndView my_sns_evas(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_evas.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		Map params = new HashMap();
		params.put("user_id", user.getId());
	...
	public ModelAndView sns_ajax_evas(HttpServletRequest request, HttpServletResponse response, String size) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/sns_ajax_evas.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		int current_size = CommUtil.null2Int(size);
		int begin = current_size * 5;
	...
	public ModelAndView my_sns_evaps(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_evaps.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		Map params = new HashMap();
		params.put("user_id", user.getId());
	...
	public ModelAndView sns_ajax_evaps(HttpServletRequest request, HttpServletResponse response, String size) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/sns_ajax_evaps.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		int current_size = CommUtil.null2Int(size);
		int begin = current_size * 5;
	...
	public ModelAndView my_sns_cons(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_cons.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		Map params = new HashMap();
		params.put("user_id", user.getId());
		List<Consult> cons = this.consultService.query("select obj from Consult obj where obj.consult_user_id = :user_id",
				params, 0, 10);
	...
	public ModelAndView sns_ajax_cons(HttpServletRequest request, HttpServletResponse response, String size) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/sns_ajax_cons.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		int current_size = CommUtil.null2Int(size);
		int begin = current_size * 5;
	...
	public ModelAndView my_sns_atts(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_atts.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		String param = "";
		SnsAttentionQueryObject qo = new SnsAttentionQueryObject(currentPage, mv, "addTime", "desc");
		qo.addQuery("obj.fromUser.id", new SysMap("user_id", user.getId()), "=");
		qo.setPageSize(16);
		IPageList pList = this.snsAttentionService.list(qo);
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/buyer/my_sns_atts.htm", "", param, pList, mv);
		mv.addObject("snsTools", snsTools);
		return mv;
	}
	...
	public ModelAndView my_sns_fans(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_fans.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = SecurityUserHolder.getCurrentUser();
		...
		qo.setPageSize(16);
		IPageList pList = this.snsAttentionService.list(qo);
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/buyer/my_sns_fans.htm", "", param, pList, mv);
		mv.addObject("snsTools", snsTools);
		return mv;
	}
	...
	public ModelAndView my_sns_circle(HttpServletRequest request, HttpServletResponse response, String type,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_circle_atten.html",
				configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(type)) { // 我创建的圈子
			mv = new JModelAndView("user/default/usercenter/sns/my_sns_circle.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			...
			if (maps.size() > 0) {
				...
				for (Map map : maps) {
					Circle cir = this.circleService.getObjById(CommUtil.null2Long(map.get("id")));
					cirs.add(cir);
				}
				InvitationQueryObject qo = new InvitationQueryObject(currentPage, mv, "addTime", "desc");
				qo.addQuery("obj.circle_id", new SysMap("circle_id", cirs.get(0).getId()), "=");
				qo.addQuery("obj.invitaion_top", new SysMap("invitaion_top", 1), "!=");// 排除置顶帖子
				...
				if (currentPage == null || CommUtil.null2Int(currentPage) == 1) {
					...
					List<CircleInvitation> tops = this.invitationService.query(
							"select obj from CircleInvitation obj where obj.invitaion_top=:invitaion_top", params, 0, 1);
					if (tops.size() > 0) {
			...
			if (user.getCircle_attention_info() != null) {
				List<Map> circle_list = Json.fromJson(List.class, user.getCircle_attention_info());
				List<Circle> objs = new ArrayList<Circle>();
				List<Map> remove_maps = new ArrayList<Map>();
				for (Map map : circle_list) {
					Circle temp = this.circleService.getObjById(CommUtil.null2Long(map.get("id")));
					if (temp != null) {
						objs.add(temp);
					} else {
						remove_maps.add(map);
				...
				if (remove_maps.size() > 0) { // 如果用户关注的圈子中有被删除的，将用户的关注圈子信息更新（将被删除的圈子信息删除）
					...
					if (circle_list.size() > 0) {
						user.setCircle_attention_info(Json.toJson(circle_list, JsonFormat.compact()));
					} else {
	...
	public ModelAndView my_sns_circle_invitation(HttpServletRequest request, HttpServletResponse response, String cid,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_circle_invitation.html",
				configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
	...
	public String my_sns_circle_invitation_operate(HttpServletRequest request, HttpServletResponse response, String operate,
			String id, String cid, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_circle.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		if ("top".equals(operate)) {
			...
			List<CircleInvitation> objs = this.invitationService.query(
					"select obj from CircleInvitation obj where obj.invitaion_top=:invitaion_top and obj.circle_id=:circle_id",
					params, -1, -1);
		...
		if ("delete".equals(operate)) {
			...
			List reply_ids = this.invitationReplyService
					.query("select obj.id from CircleInvitationReply obj where obj.invitation_id=:id", params, -1, -1);
		...
		if ("cancle_top".equals(operate)) {
			obj.setInvitaion_top(0);
			this.invitationService.update(obj);
		}
		if ("cancle_perfect".equals(operate)) {
			obj.setInvitaion_perfect(0);
			this.invitationService.update(obj);
		}
		mv.addObject("circleViewTools", circleViewTools);
		return "redirect:/buyer/my_sns_circle_invitation.htm?type=my_circle&currentPage=" + currentPage + "&cid=" + cid;
	}
	...
	public ModelAndView my_sns_invitation(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/usercenter/sns/my_sns_invitation.html",
				configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
		InvitationQueryObject qo = new InvitationQueryObject(currentPage, mv, "addTime", "desc");
		qo.addQuery("obj.user_id", new SysMap("user_id", SecurityUserHolder.getCurrentUser().getId()), "=");
		IPageList pList = this.invitationService.list(qo);
