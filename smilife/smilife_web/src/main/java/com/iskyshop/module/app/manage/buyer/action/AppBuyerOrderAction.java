...
public class AppBuyerOrderAction {
	...
	public void buyer_order(HttpServletRequest request, HttpServletResponse response, String user_id, String beginCount,
		...
		if (user != null) {
			...
			List<OrderForm> orders = this.orderFormService
					.query("select obj from OrderForm obj where obj.user_id=:user_id and obj.order_cat=:order_cat and obj.order_main=:order_main order by addTime desc",
							params, CommUtil.null2Int(beginCount), CommUtil.null2Int(selectCount));
			...
			for (OrderForm of : orders) {
				...
				order_map.put("order_total_price", this.orderFormTools.query_order_price(of.getId().toString()));// 订单总金额
				order_map.put("ship_price", of.getShip_price());// 物流费用
				if (of.getOrder_status() == 16 || of.getOrder_status() == 20) {// 当已经付款或者货到付款，查询该订单是否可以确认收货
					order_map.put("order_status", this.query_order_status(of.getId()));
				} else {
				...
				List<Map> goods_infos = orderFormTools.queryGoodsInfo(of.getGoods_info());
				List list_detail = new ArrayList();
				List name_list = new ArrayList();
				for (Map map_temp : goods_infos) {
					list_detail.add(map_temp.get("goods_mainphoto_path"));
					name_list.add(map_temp.get("goods_name"));
				}
				if (!StringUtils.isNullOrEmpty(of.getChild_order_detail())) {
					List<Map> goods_child_infos = orderFormTools.queryGoodsInfo(of.getChild_order_detail());
					for (Map map_temp : goods_child_infos) {
						List<Map> child_goods_map = orderFormTools.queryGoodsInfo(CommUtil.null2String(map_temp
								.get("order_goods_info")));
						for (Map map_temp2 : child_goods_map) {
							list_detail.add(map_temp2.get("goods_mainphoto_path"));
	...
	public void buyer_order_view(HttpServletRequest request, HttpServletResponse response, String user_id, String order_id) {
		...
		if (obj != null && user != null) {
			if (obj.getUser_id().equals(user_id)) {
				...
				json_map.put("order_total_price", CommUtil.formatMoney(this.orderFormTools.query_order_price(order_id)));// 订单总金额
				json_map.put("goods_price", CommUtil.formatMoney(this.orderFormTools.query_order_goods(order_id)));// 商品金额
				json_map.put("reduce_amount", CommUtil.null2Double(obj.getEnough_reduce_amount()));// 满减金额
				if (obj.getOrder_status() == 16 || obj.getOrder_status() == 20) {// 当已经付款或者货到付款，查询该订单是否可以确认收货
					json_map.put("order_status", this.query_order_status(obj.getId()));
				} else {
				...
				if (CommUtil.null2Int(json_map.get("order_status")) < 16) {
					...
					if (obj.getPayment() != null) {
						...
						if ("alipay_app".equals(pay_mark)) {
							json_map.put("payType", "手机App支付宝");
						}
						if ("alipay_wap".equals(pay_mark)) {
							json_map.put("payType", "手机网页支付宝");
						}
						if ("alipay".equals(pay_mark)) {
							json_map.put("payType", "支付宝");
						}
						if ("wx_app".equals(pay_mark)) {
							json_map.put("payType", "手机App微信支付");
						}
						if ("wx_pay".equals(pay_mark)) {
							json_map.put("payType", "微信支付");
						}
						if ("balance".equals(pay_mark)) {
							json_map.put("payType", "预存款");
						}
						if ("tenpay".equals(pay_mark)) {
							json_map.put("payType", "财付通");
						}
						if ("bill".equals(pay_mark)) {
							json_map.put("payType", "快钱支付");
						}
						if ("chinabank".equals(pay_mark)) {
							json_map.put("payType", "网银支付");
						}
						if ("paypal".equals(pay_mark)) {
							json_map.put("payType", "Paypal支付");
						}
					} else {
						if ("payafter".equals(obj.getPayType())) {
				...
				if (!StringUtils.isNullOrEmpty(obj.getExpress_info())) {
					Map express_map = this.orderFormTools.queryCouponInfo(obj.getExpress_info());
					map_train.put("express_company", express_map.get("express_company_name"));// 物流公司信息
				}
				...
				if (!StringUtils.isNullOrEmpty(obj.getChild_order_detail())) {
					List<Map> temp_maps = this.orderFormTools.queryGoodsInfo(obj.getChild_order_detail());
					for (Map map : temp_maps) {
						OrderForm of = this.orderFormService.getObjById(CommUtil.null2Long(map.get("order_id")));
						Map map_train2 = new HashMap();
						if (!StringUtils.isNullOrEmpty(of.getExpress_info())) {
							Map express_map = this.orderFormTools.queryCouponInfo(of.getExpress_info());
							map_train2.put("express_company", express_map.get("express_company_name"));// 物流公司信息
						}
				...
				json_map.put("ship_price", CommUtil.formatMoney(obj.getShip_price()));// 运费
				...
				if (!StringUtils.isNullOrEmpty(obj.getCoupon_info())) {
					Map coupon_map = this.orderFormTools.queryCouponInfo(obj.getCoupon_info());
					json_map.put("coupon_price", CommUtil.formatMoney(coupon_map.get("coupon_amount")));// 优惠券价格
				} else {
				...
				List<Map> temp_maps = this.orderFormTools.queryGoodsInfo(obj.getGoods_info());
				...
				for (Map goods : temp_maps) {
					...
					goods_map.put(
							"goods_gsp_val",
							StringUtils.isNullOrEmpty(goods.get("goods_gsp_val")) ? goods.get("goods_gsp_val") : goods
									.get("goods_gsp_val").toString().replaceAll("<br>", "  "));
//					goods_map.put("goods_mainphoto_path", url + "/" + goods.get("goods_mainphoto_path"));
					goods_map.put("goods_mainphoto_path", goods.get("goods_mainphoto_path"));
					goods_list.add(goods_map);
				}
				if (!StringUtils.isNullOrEmpty(obj.getChild_order_detail())) {
					List<Map> temp_maps2 = this.orderFormTools.queryGoodsInfo(obj.getChild_order_detail());
					for (Map goods : temp_maps2) {
						String child_order_id = CommUtil.null2String(goods.get("order_id"));
						OrderForm child_order = this.orderFormService.getObjById(CommUtil.null2Long(child_order_id));
						List<Map> temp_maps3 = this.orderFormTools.queryGoodsInfo(child_order.getGoods_info());
						for (Map goods3 : temp_maps3) {
							...
							goods_map.put("goods_gsp_val",
									StringUtils.isNullOrEmpty(goods3.get("goods_gsp_val")) ? goods3.get("goods_gsp_val")
											: goods3.get("goods_gsp_val").toString().replaceAll("<br>", "  "));
//							goods_map.put("goods_mainphoto_path", url + "/" + goods3.get("goods_mainphoto_path"));
							goods_map.put("goods_mainphoto_path", goods3.get("goods_mainphoto_path"));
							goods_list.add(goods_map);
						}
					}
				}
	...
	public void buyer_order_cancel(HttpServletRequest request, HttpServletResponse response, String order_id, String user_id)
			throws Exception {
		Map json_map = new HashMap();
		...
		if (obj != null && obj.getUser_id().equals(user.getId().toString())) {
			...
			if (obj.getOrder_form() == 0) {
				...
				this.msgTools.sendSmsCharge(CommUtil.getURL(request), "sms_toseller_order_cancel_notify", store.getUser()
						.getMobile(), map, null, CommUtil.null2String(store.getId()));
			}
			// 还原商品库存
	...
	public void buyer_order_cofirm(HttpServletRequest request, HttpServletResponse response, String order_id, String user_id)
			throws Exception {
		...
		if (obj != null && obj.getUser_id().equals(CommUtil.null2String(user.getId()))) {
			...
			if ("payafter".equals(obj.getPayType())) { // 如果买家支付方式为货到付款，买家确认收货时更新商品库存
			...
			if (ret) {// 订单状态更新成功，更新相关信息
				if (obj.getOrder_main() == 1 && !"".equals(CommUtil.null2String(obj.getChild_order_detail()))) { // 更新子订单状态信息
					...
					for (Map map : maps) {
						...
						this.orderFormService.update(child_order);
						if ("payafter".equals(obj.getPayType())) { // 如果买家支付方式为货到付款，买家确认收货，子订单商品销量增加
						...
						if (child_order.getOrder_form() == 0) {
							...
							if (obj.getOrder_form() == 0) {
								...
								this.msgTools.sendSmsCharge(CommUtil.getURL(request),
										"sms_toseller_order_receive_ok_notify", store.getUser().getEmail(), json_map, null,
										obj.getStore_id());
							}
							// 订单生成商家结算日志
				...
				if (obj.getOrder_form() == 0) {
					...
					if (obj.getPayType() != null && "payafter".equals(obj.getPayType())) {
						...
						plog.setTotal_amount(BigDecimal.valueOf(CommUtil.add(
								CommUtil.subtract(0, obj.getTotalPrice().subtract(obj.getShip_price())), obj.getShip_price())));// 该订单应结算金额：结算金额=订单总商品金额-总佣金费用+运费
						plog.setShip_price(obj.getShip_price()); //该订单运费
					} else {
						plog.setPl_sn("pl" + CommUtil.formatTime("yyyyMMddHHmmss", new Date()) + store.getUser().getId());
						plog.setPl_info("确认收货");
						...
						plog.setOrder_total_price(obj.getTotalPrice().subtract(obj.getShip_price()));// 该订单总商品金额
						plog.setTotal_amount(BigDecimal.valueOf(CommUtil.add(
								CommUtil.subtract(obj.getTotalPrice().subtract(obj.getShip_price()), obj.getCommission_amount()), obj.getShip_price())));// 该订单应结算金额：结算金额=订单总商品金额-总佣金费用+运费
						plog.setShip_price(obj.getShip_price()); //该订单运费
					}
					this.payoffLogservice.save(plog);
	...
	public void buyer_order_evaluate_query(HttpServletRequest request, HttpServletResponse response, String order_id,
			String user_id) {
		Map json_map = new HashMap();
...
//			goods_map.put("goods_mainphoto_path", url + "/" + goods.get("goods_mainphoto_path"));
		...
		if (!StringUtils.isNullOrEmpty(obj.getChild_order_detail())) {
			...
			for (Map goods : temp_maps2) {
				...
				for (Map goods3 : temp_maps3) {
					...
					goods_map.put("goods_price", goods3.get("goods_price"));
					goods_map.put("goods_gsp_val",
							StringUtils.isNullOrEmpty(goods3.get("goods_gsp_val")) ? goods3.get("goods_gsp_val") : goods3
									.get("goods_gsp_val").toString().replaceAll("<br>", "  "));
					goods_map.put("goods_mainphoto_path", url + "/" + goods.get("goods_mainphoto_path"));
					goods_list.add(goods_map);
				}
	...
	public void buyer_order_evaluate(HttpServletRequest request, HttpServletResponse response, String order_id,
		...
		try {
			...
			if (obj.getOrder_status() == 40) {
				...
				if (child_goods.size() > 0) {
					json.addAll(child_goods);//json里保存的是主订单及所有子订单中的商品信息，每个商品对应一个Map
				}
				for (Map map : json) {
					...
					eva.setService_evaluate(BigDecimal.valueOf(CommUtil.null2Double(request.getParameter("service_evaluate"
							+ goods.getId()))));
			...
			code = -500;
			json_map_large.put("msg", "系统后台异常!!!");
			json_map_large.put("ret", "false");
			json_map_large.put("code", code);
...
//			}
	...
	public void buyer_order_ship(HttpServletRequest request, HttpServletResponse response, String order_id) {
		Map json_map = new HashMap();
		...
		TransInfo transInfo = this.orderFormTools.query_ship_getData(order_id);
	...
	public void grouplife_order(HttpServletRequest request, HttpServletResponse response, String user_id, String beginCount) {
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		int begin = 0;
		if (beginCount != null) {
		...
		for (OrderForm obj : orders) {
			...
			map.put("order_total_price", this.orderFormTools.query_order_price(obj.getId().toString()));// 订单总金额
			...
			map.put("goods_img", goods_map.get("goods_mainphoto_path"));
			datas.add(map);
		}
	...
	public void grouplife_order_detail(HttpServletRequest request, HttpServletResponse response, String user_id, String oid) {
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		Map json_map = new HashMap();
		...
		if (obj.getOrder_status() == 10) {
			...
			json_map.put("order_pay_time", CommUtil.formatLongDate(obj.getPayTime()));
		}
		Map goods_map = this.orderformtools.queryGroupInfo(obj.getGroup_info());
		...
		json_map.put("goods_img", goods_map.get("goods_mainphoto_path"));
		List<Map> groupinfos = new ArrayList<Map>();
		Map params = new HashMap();
		params.put("user_id", CommUtil.null2Long(user_id));
		params.put("order_id", obj.getId());
		List<GroupInfo> infos = this.groupInfoService.query(
				"select obj from GroupInfo obj where obj.user_id=:user_id and obj.order_id=:order_id", params, -1, -1);
		for (GroupInfo info : infos) {
			...
			map.put("info_end_time", CommUtil.formatLongDate(info.getLifeGoods().getEndTime()));
			groupinfos.add(map);
		}
	...
	public void grouplife_order_cancel(HttpServletRequest request, HttpServletResponse response, String user_id, String oid) {
		Map json_map = new HashMap();
		OrderForm of = this.orderFormService.getObjById(CommUtil.null2Long(oid));
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
	...
	public void integral_order(HttpServletRequest request, HttpServletResponse response, String user_id, String beginCount) {
		Map json_map = new HashMap();
		Map params = new HashMap();
		params.put("igo_user_id", CommUtil.null2Long(user_id));
		List<IntegralGoodsOrder> orders = this.integralGoodsOrderService.query(
				"select obj from IntegralGoodsOrder obj where obj.igo_user.id=:igo_user_id order by addTime desc", params,
				CommUtil.null2Int(beginCount), 10);
		List list = new ArrayList();
		for (IntegralGoodsOrder of : orders) {
			...
			if (!StringUtils.isNullOrEmpty(of.getIgo_payment())) {
				...
				if ("wx_app".equals(payment)) {
					temp = "微信App";
				}
				if ("tenpay".equals(payment)) {
					temp = "财付通";
				}
				if ("bill".equals(payment)) {
					temp = "快钱";
				}
				if ("chinabank".equals(payment)) {
					temp = "网银在线";
				}
				if ("balance".equals(payment)) {
					temp = "预存款支付";
				}
			...
			List<Map> ig_maps = this.orderformtools.query_integral_goodsinfo(of.getGoods_info());
			for (Map map : ig_maps) {
				Map goods_map = new HashMap();
				...
				logger.debug("========" + map.get("ig_goods_img"));
				goods_list.add(goods_map);
			}
	...
	public void integral_order_detail(HttpServletRequest request, HttpServletResponse response, String user_id, String oid) {
		Map json_map = new HashMap();
		IntegralGoodsOrder of = this.integralGoodsOrderService.getObjById(CommUtil.null2Long(oid));
		...
		json_map.put("receiver_address", of.getReceiver_area() + of.getReceiver_area_info());
		...
		if (of.getIgo_status() == 30) {
			...
			json_map.put("express_company_name", map.get("express_company_name"));
			json_map.put("shipCode", of.getIgo_ship_code());
			json_map.put("igo_ship_content", of.getIgo_ship_content());
			json_map.put("igo_ship_time", CommUtil.formatLongDate(of.getIgo_ship_time()));
		}
		List<Map> goods_list = new ArrayList<Map>();
		List<Map> ig_maps = this.orderformtools.query_integral_goodsinfo(of.getGoods_info());
		for (Map map : ig_maps) {
	...
	public void integral_order_complete(HttpServletRequest request, HttpServletResponse response, String user_id, String oid) {
		Map json_map = new HashMap();
		IntegralGoodsOrder of = this.integralGoodsOrderService.getObjById(CommUtil.null2Long(oid));
	...
	public void integral_order_cancel(HttpServletRequest request, HttpServletResponse response, String user_id, String oid) {
		Map json_map = new HashMap();
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		IntegralGoodsOrder obj = this.integralGoodsOrderService.getObjById(CommUtil.null2Long(oid));
		if (obj != null && obj.getIgo_user().getId().equals(user.getId())) {
			obj.setIgo_status(-1);
			this.integralGoodsOrderService.update(obj);
			List<IntegralGoods> igs = this.orderformtools.query_integral_all_goods(obj.getId().toString());
			for (IntegralGoods ig : igs) {
				IntegralGoods goods = ig;
				int sale_count = this.orderformtools.query_integral_one_goods_count(obj, ig.getId().toString());
	...
	public void free_order(HttpServletRequest request, HttpServletResponse response, String user_id, String beginCount) {
		Map json_map = new HashMap();
		Map params = new HashMap();
		params.put("user_id", CommUtil.null2Long(user_id));
		List<FreeApplyLog> orders = this.freeapplylogService.query(
				"select obj from FreeApplyLog obj where obj.user_id=:user_id order by addTime desc", params,
				CommUtil.null2Int(beginCount), 10);
		List list = new ArrayList();
		for (FreeApplyLog of : orders) {
			...
			SysConfig sc = this.configService.getSysConfig();
			String img = sc.getGoodsImage().getPath() + "/" + sc.getGoodsImage().getName();
			if (fg.getFree_acc() != null) {
				order_map.put("goods_img", fg.getFree_acc().getPath() + "/" + fg.getFree_acc().getName());
			} else {
	...
	public void free_order_detail(HttpServletRequest request, HttpServletResponse response, String oid) {
		Map json_map = new HashMap();
		FreeApplyLog of = this.freeapplylogService.getObjById(CommUtil.null2Long(oid));
		Map order_map = new HashMap();
		if (!StringUtils.isNullOrEmpty(of.getExpress_info())) {
			Map map = (Map) Json.fromJson(of.getExpress_info());
			order_map.put("express_company_name", map.get("express_company_name"));
		...
		order_map.put("goods_name", fg.getFree_name());
		order_map.put("receiver_address", of.getReceiver_area() + of.getReceiver_area_info());
	...
	public void free_order_evaluate_save(HttpServletRequest request, HttpServletResponse response, String oid,
			String use_experience, String user_id) {
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		Map json_map = new HashMap();
		FreeApplyLog fal = this.freeapplylogService.getObjById(CommUtil.null2Long(oid));
	...
	public void goods_return(HttpServletRequest request, HttpServletResponse response, String user_id, String begin_count) {
		Map json_map = new HashMap();
		List<Map> datas = new ArrayList<Map>();
		...
		for (OrderForm obj : orders) {
			...
			List<Map> goods_maps_temp = this.orderFormTools.queryGoodsInfo(CommUtil.null2String(obj.getGoods_info()));
			for (Map goods : goods_maps_temp) {
				Map goods_map = new HashMap();
				goods_map.put("oid", obj.getId());
				goods_map.put("goods_id", goods.get("goods_id"));
				goods_map.put("goods_name", goods.get("goods_name"));
				goods_map.put("goods_img", goods.get("goods_mainphoto_path"));
				goods_map.put("goods_gsp_ids", CommUtil.null2String(goods.get("goods_gsp_ids")));
				String return_can = "true";
				String return_mark = "申请退货";
				String goods_return_status = CommUtil.null2String(goods.get("goods_return_status"));
				if (CommUtil.null2Int(goods.get("goods_choice_type")) == 1) {
					return_can = "false";
					return_mark = "无法退货";
				} else {
					if (goods_return_status != null) {
						if ("5".equals(goods_return_status)) {
							return_can = "false";
							return_mark = "已申请";
						} else if ("6".equals(goods_return_status)) {
							return_can = "false";
							return_mark = "填写退货物流";
						} else if ("7".equals(goods_return_status)) {
							return_can = "false";
							return_mark = "退货中";
						} else if ("8".equals(goods_return_status)) {
							return_can = "false";
							return_mark = "退货完成";
						}else if ("9".equals(goods_return_status)) {
							return_can = "false";
							return_mark = "退款完成";
						}
					}
			...
			if (obj.getChild_order_detail() != null) {
				List<Map> child_goods_maps_temp = this.orderFormTools.queryGoodsInfo(CommUtil.null2String(obj
						.getChild_order_detail()));
				for (Map c : child_goods_maps_temp) {
					//List<Map> maps = this.orderFormTools.queryGoodsInfo(c.get("order_goods_info").toString());
					OrderForm subOrderForm = this.orderFormTools.query_order(c.get("order_id").toString());
					List<Map> maps = this.orderFormTools.queryGoodsInfo(CommUtil.null2String(subOrderForm.getGoods_info()));
					for (Map goods : maps) {
						Map goods_map = new HashMap();
						goods_map.put("oid", c.get("order_id").toString());
						goods_map.put("goods_id", goods.get("goods_id"));
						goods_map.put("goods_name", goods.get("goods_name"));
						goods_map.put("goods_img", goods.get("goods_mainphoto_path"));
						goods_map.put("goods_gsp_ids", CommUtil.null2String(goods.get("goods_gsp_ids")));
						...
						if (CommUtil.null2Int(goods.get("goods_choice_type")) == 1) {
							...
							if (goods_return_status != null) {
								if ("5".equals(goods_return_status)) {
									return_can = "false";
									return_mark = "已申请";
								} else if ("6".equals(goods_return_status) || "7".equals(goods_return_status)) {
									return_can = "false";
									return_mark = "退货中";
								} else if ("8".equals(goods_return_status)) {
									return_can = "false";
		...
		json_map.put("ret", "true");
		String json = Json.toJson(json_map, JsonFormat.compact());
		logger.debug("json_map:" + json_map);
		response.setContentType("text/plain");
		response.setHeader("Cache-Control", "no-cache");
		response.setCharacterEncoding("UTF-8");
		PrintWriter writer;
	...
	public void goods_return_apply(HttpServletRequest request, HttpServletResponse response, String goods_id, String oid,
			String goods_gsp_ids, String user_id, String order_id) {
		Map json_map = new HashMap();
		OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(oid));
		if (obj.getUser_id().equals(user_id)) {
			...
			json_map.put("return_count", obj_map.get("return_goods_count") == null ? 1 : obj_map.get("return_goods_count"));
			json_map.put("oid", oid);
			json_map.put("goods_id", goods.getId());
			if ("5".equals(CommUtil.null2String(obj_map.get("goods_return_status")))) {
				...
				json_map.put("return_goods_content", return_map.get("return_goods_content"));
			}
		}
	...
	public void goods_return_apply_save(HttpServletRequest request, HttpServletResponse response, String oid,
			String return_goods_content, String goods_id, String return_goods_count, String goods_gsp_ids, String user_id,
	...
	public void goods_return_cancel_save(HttpServletRequest request, HttpServletResponse response, String oid,
			String goods_id, String goods_gsp_ids, String user_id) throws Exception {
		Map json_map = new HashMap();
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		OrderForm obj = this.orderFormService.getObjById(CommUtil.null2Long(oid));
		List<Goods> goods_list = this.orderFormTools.queryOfGoods(obj.getId().toString());
	...
	public void goods_return_ship(HttpServletRequest request, HttpServletResponse response, String oid, String goods_id,
			String user_id) {
		Map json_map = new HashMap();
		List<Map> datas = new ArrayList<Map>();
		OrderForm of = this.orderFormService.getObjById(CommUtil.null2Long(oid));
	...
	public void goods_return_ship_save(HttpServletRequest request, HttpServletResponse response, String rid, String user_id,
			String express_id, String express_code) {
		Map json_map = new HashMap();
		ReturnGoodsLog obj = this.returnGoodsLogService.getObjById(CommUtil.null2Long(rid));
		if (obj.getUser_id().toString().equals(user_id)) {
			User user = this.userService.getObjById(CommUtil.null2Long(user_id));
	...
	public void grouplife_refund(HttpServletRequest request, HttpServletResponse response, String user_id, String begin_count) {
		Map json_map = new HashMap();
		List<Map> datas = new ArrayList<Map>();
		User user = this.userService.getObjById(CommUtil.null2Long(user_id));
		Map params = new HashMap();
		...
		for (GroupInfo obj : infos) {
			...
			data.put("group_goods_img", obj.getLifeGoods().getGroup_acc().getPath() + "/"
	...
	public void grouplife_refund_apply(HttpServletRequest request, HttpServletResponse response, String group_id) {
		Map json_map = new HashMap();
	...
	public void grouplife_refund_apply_save(HttpServletRequest request, HttpServletResponse response, String group_id,
			String user_id, String return_content, String return_reasion) throws Exception {
		Map json_map = new HashMap();
		GroupInfo obj = this.groupInfoService.getObjById(CommUtil.null2Long(group_id));
		if (obj != null && obj.getUser_id().longValue() == CommUtil.null2Long(user_id).longValue()) {
			obj.setStatus(3);// 退款完成
			...
			if (order.getOrder_form() == 0) {
				...
				map.put("seller_id", seller.getId().toString());
				this.msgTools.sendEmailCharge(CommUtil.getURL(request), "email_toseller_order_refund_apply_notify",
						seller.getEmail(), map, null, order.getStore_id());
...
//			this.groupinfo_refund_complete(group_id);  //自动退款
			json_map.put("ret", "true");
		} else {
	...
	private void groupinfo_refund_complete(String group_id) {
		...
		//this.userService.update(user);
		// 保存充值日志
		String info = "<" + obj.getLifeGoods().getGg_name() + ">消费码" + obj.getGroup_sn() + "退款成功";
		PredepositLog log = new PredepositLog();
		Map params = new HashMap();
		params.put("userRole", "ADMIN");
		List<User> admins = this.userService.query("select obj from User obj where obj.userRole=:userRole", params, 0, 1);
		...
		if (of.getOrder_form() == 0) {// 商家订单生成退款结算账单
			...
			pol.setShip_price(BigDecimal.valueOf(0.0)); //退货运费为零
			...
			sc.setPayoff_all_amount(BigDecimal.valueOf(CommUtil.subtract(sc.getPayoff_all_amount(), CommUtil.mul(amount, 0))));
			...
			this.configService.update(sc);
			String msg_content = "您的团购商品：" + gfg.getGg_name() + "消费码已经成功退款，退款金额为：" + amount + "，退款消费码:" + obj.getGroup_sn()
					+ ",已存入您的账户预存款";
			// 发送系统站内信
