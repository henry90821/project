...
public class CircleCreateViewAction {
	...
	public ModelAndView circle_create(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String id) {
		ModelAndView mv = new JModelAndView("circle/circle_create.html", configService.getSysConfig(),
		...
		if (ret) {
			...
			if (user.getCircle_create_info() != null) {
				List<Map> maps = Json.fromJson(List.class, user.getCircle_create_info());
			...
			if (flag) {
				List<CircleClass> c_classes = this.circleclassService
						.query("select obj from CircleClass obj order by sequence asc", null, -1, -1);
				if (c_classes.size() > 0) {
					String session_circle_create = "session_circle_create_" + UUID.randomUUID();
					request.getSession(true).setAttribute("session_circle_create", session_circle_create);
	...
	public ModelAndView circle_create_save(HttpServletRequest request, HttpServletResponse response, String class_id,
			String img_id, String session_circle_create) {
		ModelAndView mv = new JModelAndView("success.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 1, request, response);
		String op_title = "申请成功，请等待审核";
		String session_circle_create1 = CommUtil
				.null2String(request.getSession(true).getAttribute("session_circle_create"));
		if (session_circle_create1.equals(session_circle_create)) {
			User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
			CircleClass cclass = this.circleclassService.getObjById(CommUtil.null2Long(class_id));
			...
			Accessory image = this.accessoryService.getObjById(CommUtil.null2Long(img_id));
			...
			request.getSession(true).removeAttribute("session_circle_create");
	...
	public void circle_image_upload(HttpServletRequest request, HttpServletResponse response, String img_id) {
		// 圈子图片上传
		...
		try {
			...
			if (!StringUtils.isNullOrEmpty(img_id)) { // 更新
				Accessory old_image = this.accessoryService.getObjById(CommUtil.null2Long(img_id));
				img = old_image;
				...
				img.setPath(this.ftpTools.userUpload(img.getName(), "/circle", CommUtil.null2String(user.getId())));
	...
	private int query_user_level(User user) {
		if (!StringUtils.isNullOrEmpty(this.configService.getSysConfig().getUser_level())) {
