...
public class AppPushTools {
	...
	public void ios_push(AppPushLog appPushLog) {
		ApnsService service = APNS.newService()
				.withCert(CommUtil.getServerRealPathFromSystemProp() + "resources" + File.separator + "data"
						+ File.separator + "iOSPushDev.p12", "123456")
				.withSandboxDestination() // or
				// .withProductionDestination()
				.build();

		String payload = APNS.newPayload().alertBody(appPushLog.getDescription()).sound("default").build();
		Map params = new HashMap();
		params.put("app_type", "iOS");
		List device_list = this.appPushuserService
				.query("select obj.app_id from AppPushUser obj where obj.app_type=:app_type", params, -1, -1);
	...
	public void app_chat_push(ChattingLog log) {
		...
		List<User> list = this.userService.query("select obj from User obj where obj.store.id=:store_id", params, -1, -1);
		User user = list.get(0);

		params.clear();
		params.put("user", user.getId().toString());
		List<AppPushUser> appPushUser_list = this.appPushuserService
				.query("select obj from AppPushUser obj where obj.user_id=:user", params, -1, -1);
