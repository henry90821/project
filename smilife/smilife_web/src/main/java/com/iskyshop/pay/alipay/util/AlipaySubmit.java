...
public class AlipaySubmit {
	...
	private static Map<String, String> buildRequestPara(AlipayConfig config, Map<String, String> sParaTemp)
		...
		if (!"alipay.wap.trade.create.direct".equals(sPara.get("service"))
				&& !"alipay.wap.auth.authAndExecute".equals(sPara.get("service"))) {
	...
	public static String buildForm(AlipayConfig config, Map<String, String> sParaTemp, String gateway, String strMethod,
			String strButtonName) throws UnsupportedEncodingException {
		// 待请求参数数组
	 ...
	 * @throws Exception
	 */
	public static String sendPostInfo(AlipayConfig config, Map<String, String> sParaTemp, String gateway) throws Exception {
		// 待请求参数数组
		Map<String, String> sPara = buildRequestPara(config, sParaTemp);
	...
	public static String buildRequest(AlipayConfig config, String type, Map<String, String> sParaTemp,
		...
		request.setParameters(generatNameValuePair(sPara));
		if ("web".equals(type)) {
			request.setUrl(ALIPAY_GATEWAY_NEW + "_input_charset=" + config.getInput_charset());
		}
		if ("wap".equals(type)) {
			request.setUrl(WAP_ALIPAY_GATEWAY_NEW + "_input_charset=" + config.getInput_charset());
		}
	...
	public static String getRequestToken(AlipayConfig config, String text) throws Exception {
		...
		if (paraText.get("res_data") != null) {
			...
			if ("0001".equals(config.getSign_type())) {
				res_data = RSA.decrypt(res_data, config.getPrivate_key(), config.getInput_charset());
			}
