...
public class AlipayCore {
	 ...
	 * 手机支付签名标识
	 */
	public static final String SIGN_TYPE_MOBILE = "0001";
	...
	public static String buildMysign(AlipayConfig config, Map<String, String> sArray) {
		...
		String mysign = null;
		if (SIGN_TYPE_MD5.equals(config.getSign_type())) {
			mysign = MD5.sign(prestr, config.getKey(), config.getInput_charset());
		} else if (SIGN_TYPE_RSA.equals(config.getSign_type()) || SIGN_TYPE_MOBILE.equals(config.getSign_type())) {
			mysign = RSA.sign(prestr, config.getPrivate_key(), config.getInput_charset());
		}
		return mysign;
	}
