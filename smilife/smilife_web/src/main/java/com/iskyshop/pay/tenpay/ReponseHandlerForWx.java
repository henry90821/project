...
public class ReponseHandlerForWx {
	...
	private static Logger logger = Logger.getLogger(ReponseHandlerForWx.class);
	...
	public ReponseHandlerForWx(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			AppSignature = doc.selectSingleNode("xml/AppSignature") != null
					? doc.selectSingleNode("xml/AppSignature").getText() : "";
	...
	public boolean isWXsign(String akey) {
		...
		logger.debug("加密后的：" + sign);
		logger.debug("流中获取的:" + this.AppSignature);
