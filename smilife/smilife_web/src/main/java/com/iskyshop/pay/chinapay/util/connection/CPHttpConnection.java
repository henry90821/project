...
public abstract class CPHttpConnection {
	...
	public int sendMsg(String msgStr) {
		...
		try {
			...
			LOGGER.error("连接失败,errCode=[-12];errMsg=[无法连接对方主机]");
			return -12;
		} catch (IOException ioe) {
			// TraceLog.logStackTrace(this, (Throwable) ioe);
			LOGGER.error("连接失败,errCode=[-14];errMsg=[通讯建链失败]");
			return -14;
		}
		...
		try {
			...
			if (StringUtils.isNullOrEmpty(getMsgEncoding())) {
			...
			LOGGER.error("Http连接失败,errCode=[-52];errMsg=[数据处理超时]");
			return -52;
		} catch (Exception e) {
			LOGGER.error("数据处理失败");
			return -1;
		...
		try {
			...
			if (RESPCODE_SUCCESS == respCode) {
				...
				LOGGER.error(new StringBuffer("对方返回错误代码=[").append(respCode).append("];respMsg=[")
						.append(urlCon.getResponseMessage()).append("]").toString());
			}

		} catch (IOException ioex) {
			LOGGER.error("连接失败,errCode=[-52];errMsg=[数据接收错误]");
			return -54;
		} catch (Exception ex) {
			LOGGER.error("系统处理异常");
			return -1;
