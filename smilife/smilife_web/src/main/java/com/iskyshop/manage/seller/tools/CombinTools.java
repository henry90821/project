...
public class CombinTools {
	...
	public Map getMainGoodsMap(String plan_id) {
		...
		CombinPlan obj = this.combinplanService.getObjById(CommUtil.null2Long(plan_id));
	...
	public String getStoreName(String plan_id) {
		...
		if (obj.getCombin_form() == 1) {
			...
			if (store != null) {
				store_name = (store != null ? store.getStore_name() : "");
			}
		}
		return store_name;
	}
		...
		if (cart != null) {
			if (cart.getCart_type() != null && "combin".equals(cart.getCart_type()) && cart.getCombin_main() == 1) {
				...
				for (String cart_id : cart_ids) {
					if (!StringUtils.isNullOrEmpty(cart_id) && !cart_id.equals(id)) {
