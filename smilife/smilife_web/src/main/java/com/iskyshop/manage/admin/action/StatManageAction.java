...
public class StatManageAction {
	...
	public ModelAndView stat_user(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_user.html", configService.getSysConfig(),
	...
	public ModelAndView stat_user_done(HttpServletRequest request, HttpServletResponse response, String beginTime,
			String endTime) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_user_result.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		if (begin != null && end != null) {
			...
			if (day > 0 && day <= 90) {
				...
				List<User> users = this.userService.query(
						"select obj.id from User obj where obj.addTime>=:addTime and obj.addTime<:addTime1", params, -1, -1);
				us.setUser_increase_count(users.size());
				params.clear();
				params.put("addTime1", cal1.getTime());
				users = this.userService.query("select obj.id from User obj where obj.addTime<:addTime1", params, -1, -1);
				us.setUser_count(users.size());
				params.clear();
				params.put("addTime", begin);
				params.put("addTime1", cal1.getTime());
				users = this.userService.query(
						"select obj.id from User obj where obj.loginDate>=:addTime and obj.loginDate<:addTime1", params, -1,
						-1);
				us.setUser_active_count(users.size());
				List<String> user_ids = this.orderFormService.queryFromOrderForm(
				...
				for (String user_id : user_ids) {
					...
					if (user.getAddTime().after(begin) && user.getAddTime().before(cal1.getTime())) {
				...
				for (int i = 0; i <= day - 1; i++) { // 统计区间所有时间段
					...
					params.put("addTime1", cal1.getTime());
					users = this.userService.query(
							"select obj.id from User obj where obj.addTime>=:addTime and obj.addTime<:addTime1", params, -1,
							-1);
					us.setUser_increase_count(users.size());
					params.clear();
					params.put("addTime1", cal1.getTime());
					users = this.userService.query("select obj.id from User obj where obj.addTime<:addTime1", params, -1,
							-1);
					us.setUser_count(users.size());
					params.clear();
					params.put("addTime", cal.getTime());
					params.put("addTime1", cal1.getTime());
					users = this.userService.query(
							"select obj.id from User obj where obj.loginDate>=:addTime and obj.loginDate<:addTime1", params,
							-1, -1);
					us.setUser_active_count(users.size());
					user_ids = this.orderFormService.queryFromOrderForm(
							"select distinct(obj.user_id) from OrderForm obj where obj.payTime>=:addTime and obj.payTime<:addTime1",
							params, -1, -1);
					us.setUser_order_count(user_ids.size());
					for (String user_id : user_ids) {
				...
				mv.addObject("user_counts", Json.toJson(user_counts, JsonFormat.compact()));
				mv.addObject("user_increase_counts", Json.toJson(user_increase_counts, JsonFormat.compact()));
				mv.addObject("user_active_counts", Json.toJson(user_active_counts, JsonFormat.compact()));
				mv.addObject("user_order_counts", Json.toJson(user_order_counts, JsonFormat.compact()));
				mv.addObject("user_day_order_counts", Json.toJson(user_day_order_counts, JsonFormat.compact()));
			} else if (day < 0) {
				mv = new JModelAndView("admin/blue/error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
				mv.addObject("op_title", "结束日期必须迟于开始日期");
				mv.addObject("list_url", CommUtil.getURL(request) + "/admin/stat_user.htm");
			} else if (day > 90) {
				mv = new JModelAndView("admin/blue/error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView stat_order(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_order.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView stat_order_done(HttpServletRequest request, HttpServletResponse response, String beginTime,
			String endTime) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_order_result.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Date begin = CommUtil.formatDate(beginTime);
		Date end = CommUtil.formatDate(endTime);
		if (begin != null && end != null) {
			...
			if (day > 0 && day <= 90) {
				...
				List<OrderForm> ofs = this.orderFormService.query(
						"select obj.id from OrderForm obj where obj.addTime>=:addTime and obj.addTime<:addTime1", params, -1,
						-1);
				os.setOrder_count(ofs.size());
				ofs = this.orderFormService.query(
						"select obj.id from OrderForm obj where obj.payTime>=:addTime and obj.payTime<:addTime1", params, -1,
						-1);
				os.setOrder_pay_count(ofs.size());
				ofs = this.orderFormService.query(
						"select obj.id from OrderForm obj where obj.shipTime>=:addTime and obj.shipTime<:addTime1", params,
						-1, -1);
				os.setOrder_ship_count(ofs.size());
				List amounts = this.orderFormService.queryFromOrderForm(
				...
				for (int i = 0; i <= day - 1; i++) { // 统计区间所有时间段
					...
					ofs = this.orderFormService.query(
							"select obj.id from OrderForm obj where obj.addTime>=:addTime and obj.addTime<:addTime1", params,
							-1, -1);
					os.setOrder_count(ofs.size());
					ofs = this.orderFormService.query(
							"select obj.id from OrderForm obj where obj.payTime>=:addTime and obj.payTime<:addTime1", params,
							-1, -1);
					os.setOrder_pay_count(ofs.size());
					ofs = this.orderFormService.query(
							"select obj.id from OrderForm obj where obj.shipTime>=:addTime and obj.shipTime<:addTime1",
							params, -1, -1);
					os.setOrder_ship_count(ofs.size());
					amounts = this.orderFormService.queryFromOrderForm(
							"select sum(obj.totalPrice) from OrderForm obj where obj.addTime>=:addTime and obj.addTime<:addTime1 ",
							params, -1, -1);
					os.setOrder_amount(CommUtil.null2Float(amounts.get(0)));
				...
				mv.addObject("times", Json.toJson(times, JsonFormat.compact()));
				mv.addObject("timeslength", CommUtil.null2Int(times.size() / 9));
				mv.addObject("order_counts", Json.toJson(order_counts, JsonFormat.compact()));
				mv.addObject("order_ship_counts", Json.toJson(order_ship_counts, JsonFormat.compact()));
				mv.addObject("order_pay_counts", Json.toJson(order_pay_counts, JsonFormat.compact()));
				mv.addObject("order_amounts", Json.toJson(order_amounts, JsonFormat.compact()));
	...
	public ModelAndView stat_visit(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_visit.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView stat_goods(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_goods.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		List<GoodsBrand> gbs = this.goodsBrandService.query("select obj from GoodsBrand obj order by obj.sequence asc", null,
				-1, -1);
	...
	public ModelAndView stat_goods_list(HttpServletRequest request, HttpServletResponse response, String beginTime,
			String endTime, String currentPage, String goods_name, String class_id, String brand_id) {
		ModelAndView mv = null;
		if (!StringUtils.isNullOrEmpty(endTime) && !StringUtils.isNullOrEmpty(beginTime)) {
			mv = new JModelAndView("admin/blue/stat_goods_list.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			...
			if (!StringUtils.isNullOrEmpty(class_id)) {
				Set<Long> ids = this.genericIds(this.goodsClassService.getObjById(CommUtil.null2Long(class_id)));
				para.put("ids", ids);
			...
			if (nowpage == 0) {
				...
				if (nowpage > pages) {
					cp = pages;
					newlist = this.goodsLogService.query(hql, para, (pages - 1) * pagesize, pagesize);
				} else {
					newlist = this.goodsLogService.query(hql, para, (nowpage - 1) * pagesize, pagesize);
				}
			}

			mv.addObject("imageTools", imageTools);
			String url = CommUtil.getURL(request) + "/admin/stat_goods_list.htm";
			mv.addObject("goodslog_list", newlist);
			mv.addObject("goodslog_list_gotoPageAjaxHTML", CommUtil.showPageAjaxHtml(url, "", cp, pages));
		}
		return mv;
	}
	...
	public ModelAndView stat_goods_done(HttpServletRequest request, HttpServletResponse response, String beginTime,
			String endTime, String ids, String statType) {
		ModelAndView mv = null;
		if (!StringUtils.isNullOrEmpty(ids)) {
			...
			if (arr.length == 1) { // 一件商品的统计
				mv = new JModelAndView("admin/blue/stat_single_goods_result.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
				if (begin != null && end != null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(begin);
					...
					while (!cal.getTime().after(end)) {
						...
						log = this.goodsLogService.query(
						...
						for (GoodsLog gl : log) {
							...
							price = price == 0 ? CommUtil.null2Double(gl.getPrice())
									: (price + CommUtil.null2Double(gl.getPrice())) / 2;

							if (!"".equals(gl.getPreferential())) {

								if (preferential.size() == 0) {
									Map todaytpre = new HashMap();
									todaytpre.put(gl.getPreferential(), gl.getPreferential_info());
									preferential.add(todaytpre);
								} else {

									Map temppre = preferential.get(preferential.size() - 1);
									if (!(temppre.containsKey(gl.getPreferential())
											&& temppre.get(gl.getPreferential()).equals(gl.getPreferential_info()))) {
										Map todaytpre = new HashMap();
										todaytpre.put(gl.getPreferential(), gl.getPreferential_info());
										preferential.add(todaytpre);
									}
							...
							if (!StringUtils.isNullOrEmpty(jsonstr)) {
								Map<String, Integer> clickmap = (Map<String, Integer>) Json.fromJson(jsonstr);
								it = clickmap.keySet().iterator();
								while (it.hasNext()) {
									String key = it.next().toString();
									if (goods_clickfrom.containsKey(key)) {
										goods_clickfrom.put(key, goods_clickfrom.get(key) + clickmap.get(key));
									} else {
							...
							if (!StringUtils.isNullOrEmpty(jsonstr)) {
								Map<String, Integer> ordermap = (Map<String, Integer>) Json.fromJson(jsonstr);
								...
								while (it.hasNext()) {
									...
									if (goods_ordertype.containsKey(from)) {
										goods_ordertype.put(from, goods_ordertype.get(from) + ordermap.get(key));
									} else {
							...
							if (!StringUtils.isNullOrEmpty(jsonstr)) {
								Map<String, Integer> specmap = (Map<String, Integer>) Json.fromJson(jsonstr);
								it = specmap.keySet().iterator();
								while (it.hasNext()) {
									String key = it.next().toString();
									if (goods_spectype.containsKey(key)) {
										goods_spectype.put(key, goods_spectype.get(key) + specmap.get(key));
									} else {
										goods_spectype.put(key, specmap.get(key));
									}
								}
							}
						}
					...
					while (it.hasNext()) {
						...
						clicksb.append("['").append(string).append("',").append(goods_clickfrom.get(string)).append("],");
					}
					...
					while (it.hasNext()) {
						String string = (String) it.next();
						ordersb.append("['").append(string).append("',").append(goods_ordertype.get(string)).append("],");
					}
					...
					while (it.hasNext()) {
						String string = (String) it.next();
						if ("".equals(string)) {
							specsb.append("['").append("默认规格").append("',").append(goods_spectype.get(string)).append("],");
						} else {

							specsb.append("['").append(string).append("',").append(goods_spectype.get(string)).append("],");
						}
					}
					...
					mv.addObject("times", Json.toJson(times, JsonFormat.compact()));
					mv.addObject("timeslength", CommUtil.null2Int(times.size() / 9));
					mv.addObject("goods_click", Json.toJson(goods_click, JsonFormat.compact()));
					mv.addObject("goods_collect", Json.toJson(goods_collect, JsonFormat.compact()));
					mv.addObject("goods_salenum", Json.toJson(goods_salenum, JsonFormat.compact()));
					mv.addObject("goods_clickfrom", clicksb);
					mv.addObject("goods_ordertype", ordersb);
					mv.addObject("goods_spectype", specsb);
				}
			} else { // 2个及以上商品
				mv = new JModelAndView("admin/blue/stat_multi_goods_result.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
				if (begin != null && end != null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(begin);
						...
						for (String id : arr) {
							...
							for (GoodsLog gl : log) {
								...
								if (clickcount.containsKey(tempid)) {
									clickcount.put(tempid, clickcount.get(tempid) + gl.getGoods_click());
								} else {
									clickcount.put(tempid, gl.getGoods_click());
								}

								if (salecount.containsKey(tempid)) {
									salecount.put(tempid, salecount.get(tempid) + gl.getGoods_salenum());
								} else {
									salecount.put(tempid, gl.getGoods_salenum());
								}
	...
	public ModelAndView stat_all(HttpServletRequest request, HttpServletResponse response, String beginTime, String endTime,
		...
		mv = new JModelAndView("admin/blue/stat_all_goods_result.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (begin != null && end != null) {
			...
			while (!cal.getTime().after(end)) {
				...
				for (GoodsLog gl : log) {
					...
					if (clickcount.containsKey(id)) {
						clickcount.put(id, clickcount.get(id) + gl.getGoods_click());
					} else {
						clickcount.put(id, gl.getGoods_click());
					}

					if (salecount.containsKey(id)) {
						salecount.put(id, salecount.get(id) + gl.getGoods_salenum());
					} else {
	...
	public ModelAndView stat_area(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_area.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		return mv;
	}
	...
	public ModelAndView stat_area_done(HttpServletRequest request, HttpServletResponse response, String beginTime,
			String endTime) {
		ModelAndView mv = new JModelAndView("admin/blue/stat_area_result.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		// 订单地域分布
		Object formAreaDataTemp = this.orderFormService
				.query("select substring(receiver_area,1,2),count(*) from OrderForm obj where obj.order_main =:order_main "
						+ "and obj.order_status>=:order_status and obj.finishTime>=:beginDate and obj.finishTime<:endDate "
						+ "group by substring(receiver_area,1,2) order by count(*) desc", params, -1, -1);
		...
		for (int i = 0; i < formAreaDatas.size(); i++) {
			if (i < 6) {
				...
				for (int j = 0; j < formAreaData.length; j++) {
					if (j == 0) {
						if ("黑龙".equals(formAreaData[j])) {
							lineDistrictData.append("'").append("黑龙江").append("',");
						} else {
							lineDistrictData.append("'").append(formAreaData[j]).append("',");
						}
						mapCityData.append("'").append(formAreaData[j]).append("':'").append(this.getDistrictColor(i))
								.append("',");
					} else {
