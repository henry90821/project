...
public class ExpressCompanyCommonSellerAction {
	...
	public ModelAndView ecc_set(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy,
			String orderType) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_set.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Map params = new HashMap();
		params.put("company_status", 0);
		List<ExpressCompany> ecs = this.expressCompanyService.query(
				"select obj from ExpressCompany obj where obj.company_status=:company_status order by obj.company_sequence asc",
				params, -1, -1);
	...
	public void ecc_save(HttpServletRequest request, HttpServletResponse response, String ids) {
		...
		for (ExpressCompanyCommon ecc : eccs) {// 首先删除邮件保存但是此次没有勾选的常用快递物流
			...
			for (String ec_id : ec_ids) {
				if (!"".equals(CommUtil.null2String(ec_id))) {
	...
	public ModelAndView ecc_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
	...
	public String ecc_delete(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
		ExpressCompanyCommon ecc = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
	...
	public void ecc_default(HttpServletRequest request, HttpServletResponse response, String id) {
		...
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		if (!"".equals(CommUtil.null2String(id)) && obj.getEcc_store_id().equals(store.getId())) {
			Map params = new HashMap();
	...
	public ModelAndView ecc_print_view(HttpServletRequest request, HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_print_view.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		...
		if (obj != null && obj.getEcc_store_id().equals(store.getId())) {
			Map offset_map = Json.fromJson(Map.class, obj.getEcc_template_offset());
			if ("".equals(CommUtil.null2String(obj.getEcc_template()))) {
	...
	public ModelAndView ecc_print_set(HttpServletRequest request, HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_print_set.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
	...
	public void ecc_print_set_save(HttpServletRequest request, HttpServletResponse response, String id, String left_offset,
			String top_offset) {
		boolean ret = true;
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		...
		if (obj != null && obj.getEcc_store_id().equals(store.getId())) {
			...
			if (offset_map.get("receipt_user_left") != null) {
				...
				obj.setEcc_template_offset(Json.toJson(offset_map, JsonFormat.compact()));
				ret = this.expressCompanyCommonService.update(obj);
			}
		}
		response.setContentType("text/plain");
	...
	public ModelAndView ecc_create(HttpServletRequest request, HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_create.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
	...
	public ModelAndView ecc_template_save(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage, String ecc_template_width, String ecc_template_heigh) {
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(Long.parseLong(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
		if (obj != null && obj.getEcc_store_id().equals(store.getId())) {
			...
			try {
				...
				if (map.get("fileName") != "") {
					obj.setEcc_template(this.FTPTools.systemUpload(CommUtil.null2String(map.get("fileName")), "/ecc_template/")
	...
	public ModelAndView ecc_bind_defalut_template(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(Long.parseLong(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
		if (obj != null && obj.getEcc_store_id().equals(store.getId())) {
			ExpressCompany ec = this.expressCompanyService.getObjById(obj.getEcc_ec_id());
			...
			ModelAndView mv = new JModelAndView("user/default/sellercenter/seller_error.html?currentPage=" + currentPage,
					configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
			mv.addObject("op_title", "物流参数错误，无法设置");
			mv.addObject("url", CommUtil.getURL(request) + "/seller/ecc_list.htm?currentPage=" + currentPage);
			return mv;
	...
	public ModelAndView ecc_design(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/ecc_design.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		ExpressCompanyCommon obj = this.expressCompanyCommonService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
		if (obj != null && obj.getEcc_store_id().equals(store.getId())) {
			Map offset_map = Json.fromJson(Map.class, CommUtil.null2String(obj.getEcc_template_offset()));
			mv.addObject("offset_map", offset_map);
			mv.addObject("obj", obj);
