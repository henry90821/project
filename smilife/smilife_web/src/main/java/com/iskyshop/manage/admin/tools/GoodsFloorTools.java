...
public class GoodsFloorTools {
				...
				for (Map map : list) {					
					...
					if (the_gc != null) {						
						...
						if(count > 0) {
							...
							List<GoodsClass> childs = this.goodsClassService.query("select new GoodsClass(id,className) from GoodsClass obj where obj.id in(:ids)", params, -1, -1);
	...
	public Map generic_goods_list(String json) {
		...
		if (!StringUtils.isNullOrEmpty(json)) {
			try {
				...
				map.put("goods7", this.goodsService.getObjById(CommUtil.null2Long(list.get("goods_id7"))));
				map.put("goods8", this.goodsService.getObjById(CommUtil.null2Long(list.get("goods_id8"))));
				map.put("goods9", this.goodsService.getObjById(CommUtil.null2Long(list.get("goods_id9"))));
				map.put("goods10", this.goodsService.getObjById(CommUtil.null2Long(list.get("goods_id10"))));
			} catch (Exception e) {
				map.put("list_title", "");
				map.put("goods1", null);
				map.put("goods2", null);
			...
			try {
				...
				if ("".equals(CommUtil.null2String(map.get("adv_id")))) {
					...
					if (img != null) {
						...
						template = template + "<a href='" + url + "' target='_blank'><img src='" + img.getPath() + "/"
								+ img.getName() + "' /></a>";
					}
				} else {
	...
	public List<Information> generic_info(String json) {
		...
		if (!StringUtils.isNullOrEmpty(json)) {
			try {
				...
				for (int i = 1; i <= 14; i++) {
					String key = "info" + i;
					Information brand = this.informationService.getObjById(CommUtil.null2Long(map.get(key)));
					if (brand != null) {
						infos.add(brand);
					}
				}
		...
		if (!StringUtils.isNullOrEmpty(json)) {
			try {
				...
				for (int i = 1; i <= 6; i++) {
					...
					FreeGoods free = this.freeGoodsService.getObjById(CommUtil.null2Long(map.get(key)));
					if (free != null) {
						frees.add(free);
					}
				}
		...
		if (!StringUtils.isNullOrEmpty(json)) {
			try {
				...
				for (int i = 1; i <= 8; i++) {
					...
					Circle free = this.circleService.getObjById(CommUtil.null2Long(map.get(key)));
					if (free != null) {
						circles.add(free);
					}
				}
	...
	public Map generic_style2_goods(String json, String module_id) {
		...
		if (StringUtils.isNullOrEmpty(json) || StringUtils.isNullOrEmpty(module_id)) {
			logger.warn("GoodsFloorTools.generic_style2_goods(json, module_id) -------> json parameter is null");
			return null;
		}
		try {
	...
	public Map generic_info_list(String json) {
		...
		if (!StringUtils.isNullOrEmpty(json)) {
			try {
				Map list = Json.fromJson(Map.class, json);
				map.put("list_title", CommUtil.null2String(list.get("list_title")));
				map.put("info1", this.informationService.getObjById(CommUtil.null2Long(list.get("info1"))));
				map.put("info2", this.informationService.getObjById(CommUtil.null2Long(list.get("info2"))));
				map.put("info3", this.informationService.getObjById(CommUtil.null2Long(list.get("info3"))));
				map.put("info4", this.informationService.getObjById(CommUtil.null2Long(list.get("info4"))));
				map.put("info5", this.informationService.getObjById(CommUtil.null2Long(list.get("info5"))));
				map.put("info6", this.informationService.getObjById(CommUtil.null2Long(list.get("info6"))));
				map.put("info7", this.informationService.getObjById(CommUtil.null2Long(list.get("info7"))));
				map.put("info8", this.informationService.getObjById(CommUtil.null2Long(list.get("info8"))));
				map.put("info9", this.informationService.getObjById(CommUtil.null2Long(list.get("info9"))));
				map.put("info10", this.informationService.getObjById(CommUtil.null2Long(list.get("info10"))));
			} catch (Exception e) {
				map.put("list_title", "");
				map.put("info1", null);
				map.put("info2", null);
