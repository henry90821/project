...
public class StoreAdjustApplyAction {
	...
	public ModelAndView adjust_goodsclass(HttpServletRequest request, HttpServletResponse response) {
		...
		List<StoreAdjustInfo> infos = this.adjustInfoService
				.query("select obj from StoreAdjustInfo obj where obj.store_id=:store_id", map, -1, -1);
		if (infos.size() == 0) {
			mv = new JModelAndView("user/default/sellercenter/adjust_goodsclass.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			List<StoreGrade> grades = this.storeGradeService
					.query("select obj from StoreGrade obj order by obj.sequence asc", null, -1, -1);
			...
			mv = new JModelAndView("user/default/sellercenter/adjust_info.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void add_gc_detail(HttpServletRequest request, HttpServletResponse response, String grade_id, String gc_main_id) {
		String jsonList = "";
	...
	public void adjust_gc_save(HttpServletRequest request, HttpServletResponse response, String ids, String gc_detail_info) {
	...
	public void adjust_gc_del(HttpServletRequest request, HttpServletResponse response, String id, String gc_detail_info) {
		GoodsClass gc = this.goodsclassService.getObjById(CommUtil.null2Long(id));
	...
	public ModelAndView adjust_apply_save(HttpServletRequest request, HttpServletResponse response, String gc_main,
			String gc_detail_info, String store_grade) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
		ModelAndView mv = null;
		...
		if (infos.size() == 0) {
			...
			if(!StringUtils.isNullOrEmpty(gc_detail_info)){
				...
				storeAdjustInfo.setAdjust_gc_info("{}");
			}
