...
public class CrmAspectTools {
	...
	public void order_aop(JoinPoint point) {
		if (point.getArgs().length > 0) {
			if ("save".equals(point.getSignature().getName())) {
				OrderForm of = (OrderForm) point.getArgs()[0];
				if (of != null && of.getId() != null) {
					if (of.getOrder_cat() == 0) {
						...
						List<Map> list = Json.fromJson(List.class, of.getGoods_info());
						if (list.size() > 0) {
							crm.setGoods_id(CommUtil.null2Long(list.get(0).get("goods_id")));
							crm.setGoods_name(list.get(0).get("goods_name").toString());
						}
		...
		if (point.getArgs().length > 0) {
			if ("save".equals(point.getSignature().getName())) {
				...
				if (c != null && c.getId() != null) {
					...
					List<Map> list = Json.fromJson(List.class, c.getGoods_info());
					if (list.size() > 0) {
						crm.setGoods_id(CommUtil.null2Long(list.get(0).get("goods_id")));
						crm.setGoods_name(list.get(0).get("goods_name").toString());
					}
