...
public class FreeGoodsSellerAction {
	...
	public ModelAndView freegoods_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String free_name, String beginTime, String endTime, String cls,
			String status) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/freegoods_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		SysConfig config = this.configService.getSysConfig();
		if (config.getWhether_free() == 1 && user.getStore().getGrade().getWhether_free() == 1) {
	...
	public ModelAndView freegoods_add(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/freegoods_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		SysConfig config = this.configService.getSysConfig();
	...
	public void freegoods_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage,
		...
		if (config.getWhether_free() == 1 && user.getStore().getGrade().getWhether_free() == 1) {
			...
			try {
				...
				if (StringUtils.isNullOrEmpty(fileName)) {
					if (map.get("fileName") != "") {
						...
						free_acc.setPath(
								this.ftpTools.userUpload(free_acc.getName(), "/free", CommUtil.null2String(user.getId())));
			...
			json.put("url", CommUtil.getURL(request) + "/seller/freegoods_list.htm");
	...
	public ModelAndView free_goods(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/free_goods.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView freegoods_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/freegoods_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		...
		if (config.getWhether_free() == 1 && user.getStore().getGrade().getWhether_free() == 1) {
			if (!StringUtils.isNullOrEmpty(id)) {
				...
				if (freegoods.getFreeStatus() == -5 || freegoods.getFreeStatus() == 0) {
					...
					mv.addObject("url", CommUtil.getURL(request) + "/seller/freegoods_list.htm");
				}
			} else {
				mv = new JModelAndView("user/default/sellercenter/seller_error.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void free_goods_load(HttpServletRequest request, HttpServletResponse response, String goods_name, String gc_id) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
	...
	public String free_goods_del(HttpServletRequest request, HttpServletResponse response, String mulitId,
			String currentPage) {
		...
		for (String id : ids) {
			if (!StringUtils.isNullOrEmpty(id)) {
				FreeGoods freegoods = this.freegoodsService.getObjById(Long.parseLong(id));
				if (freegoods != null && freegoods.getFreeStatus() != 5 && freegoods.getFreeType() == 0
						&& freegoods.getStore_id().equals(user.getStore().getId())) {
	...
	public ModelAndView apply_logs(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String id, String userName, String status) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/freeapplylog_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		SysConfig config = this.configService.getSysConfig();
		FreeGoods fg = this.freegoodsService.getObjById(CommUtil.null2Long(id));
		if (fg != null && fg.getStore_id().equals(user.getStore().getId()) && config.getWhether_free() == 1
			...
			CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/seller/apply_logs.htm", "", params, pList, mv);
		} else {
			mv = new JModelAndView("user/default/sellercenter/seller_error.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			mv.addObject("op_title", "系统未开启0元试用功能");
	...
	public ModelAndView apply_log_info(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/freeapplylog_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(id)) {
	...
	public void apply_log_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage,
		...
		if (config.getWhether_free() == 1 && user.getStore().getGrade().getWhether_free() == 1) {
			...
			if ("yes".equals(status)) {
				ExpressCompany ec = this.expressCompayService.getObjById(CommUtil.null2Long(ecc_id));
			...
			} else if (freeapplylog.getStore_id().equals(user.getStore().getId())) {
				this.freeapplylogService.update(freeapplylog);
			}

			json.put("ret", true);
			json.put("op_title", "修改0元试用申请状态成功");
			json.put("url", CommUtil.getURL(request) + "/seller/apply_logs.htm?id=" + freeapplylog.getFreegoods_id());
			this.return_json(Json.toJson(json, JsonFormat.compact()), response);
		} else {
			json.put("op_title", "系统未开启0元试用功能");
			json.put("url", CommUtil.getURL(request) + "/seller/index.htm");
