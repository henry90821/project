...
public class SelfFreeGoodsManageAction {
	...
	public ModelAndView self_freegoods_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String free_name, String beginTime, String endTime, String cls,
			String status) {
		ModelAndView mv = new JModelAndView("admin/blue/self_freegoods_list.html", configService.getSysConfig(),
		...
		if (!StringUtils.isNullOrEmpty(cls)) {
			qo.addQuery("obj.class_id", new SysMap("class_id", CommUtil.null2Long(cls)), "=");
			mv.addObject("cls_id", cls);
		}
		if (status != null && "going".equals(status)) {
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/admin/self_freegoods_list.htm", "", params, pList, mv);
		List<FreeClass> fcls = this.freeClassService.query("select obj from FreeClass obj", null, -1, -1);
	...
	public ModelAndView self_freegoods_add(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/self_freegoods_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("currentPage", currentPage);
		List<FreeClass> cls = this.freeClassService.query("select obj from FreeClass obj", null, -1, -1);
	...
	public ModelAndView self_freegoods_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/self_freegoods_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(id)) {
			FreeGoods freegoods = this.freegoodsService.getObjById(Long.parseLong(id));
			List<FreeClass> cls = this.freeClassService.query("select obj from FreeClass obj", null, -1, -1);
			mv.addObject("cls", cls);
			if (freegoods != null) {
				Goods goods = this.goodsService.getObjById(freegoods.getClass_id());
	...
	public ModelAndView self_freegoods_save(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage, String cmd, String list_url, String add_url, String class_id, String goods_id,
		...
		try {
			...
			if (StringUtils.isNullOrEmpty(fileName)) {
				if (map.get("fileName") != "") {
					...
					free_acc.setPath(this.FTPTools.systemUpload(free_acc.getName(), "/free"));
		...
		ModelAndView mv = new JModelAndView("admin/blue/success.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("list_url", CommUtil.getURL(request) + "/admin/self_freegoods_list.htm");
		mv.addObject("op_title", "保存0元试用成功");
		mv.addObject("add_url", CommUtil.getURL(request) + "/admin/self_freegoods_add.htm" + "?currentPage=" + currentPage);
		return mv;
	}
	...
	public String self_freegoods_del(HttpServletRequest request, HttpServletResponse response, String mulitId,
			String currentPage) {
		String[] ids = mulitId.split(",");
		for (String id : ids) {
			if (!StringUtils.isNullOrEmpty(id)) {
				FreeGoods freegoods = this.freegoodsService.getObjById(Long.parseLong(id));
				if (freegoods != null && freegoods.getFreeStatus() != 5 && freegoods.getFreeType() == 1) {
					...
					try {
						this.FTPTools.systemDeleteFtpImg(freegoods.getFree_acc());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						logger.error(e);
					}
	...
	public ModelAndView self_freeapply_logs(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String id, String userName, String status) {
		ModelAndView mv = new JModelAndView("admin/blue/self_freeapplylog_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		String params = "";
		FreeApplyLogQueryObject qo = new FreeApplyLogQueryObject(currentPage, mv, orderBy, orderType);
		qo.addQuery("obj.freegoods_id", new SysMap("freegoods_id", CommUtil.null2Long(id)), "=");
		if (!StringUtils.isNullOrEmpty(userName)) {
			qo.addQuery("obj.user_name", new SysMap("userName", "%" + userName + "%"), "like");
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/admin/self_freeapply_logs.htm", "", params, pList, mv);
		mv.addObject("id", id);
		return mv;
	}
	...
	public ModelAndView self_apply_log_info(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/self_freeapplylog_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(id)) {
			FreeApplyLog freeapplylog = this.freeapplylogService.getObjById(Long.parseLong(id));
			...
			List<ShipAddress> shipAddrs = this.shipAddressService.query(
					"select obj from ShipAddress obj where obj.sa_type=:sa_type order by obj.sa_default desc,obj.sa_sequence asc",
	...
	public ModelAndView self_apply_log_save(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage, String status, String shipCode, String ec_id, String sa_id) {
		WebForm wf = new WebForm();
		...
		if (StringUtils.isNullOrEmpty(id)) {
			...
			FreeApplyLog obj = this.freeapplylogService.getObjById(Long.parseLong(id));
		...
		if ("yes".equals(status)) {
			...
			freeapplylog.setShip_addr(area.getParent().getParent().getAreaName() + area.getParent().getAreaName()
		...
		mv.addObject("list_url",
				CommUtil.getURL(request) + "/admin/self_freeapply_logs.htm?id=" + freeapplylog.getFreegoods_id());
		mv.addObject("op_title", "审核申请成功");
		return mv;
	}

	@RequestMapping("/admin/self_freegoods_ajax.htm")
	public void freegoods_ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName,
	...
	public ModelAndView free_goods_self(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/free_goods_self.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void free_goods_self_load(HttpServletRequest request, HttpServletResponse response, String goods_name,
			String gc_id) {
		boolean ret = true;
		Map params = new HashMap();
		...
		String query = "select obj from Goods obj where obj.goods_name like:goods_name and obj.goods_type=:goods_type and obj.goods_status=:goods_status and obj.whether_free=:whether_free and"
				+ " obj.seckill_buy=:seckill_buy";
