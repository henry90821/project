...
public class SmsManageAction {
	...
	public ModelAndView sms_set(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/sms_set.html", configService.getSysConfig(),
	...
	public ModelAndView sms_set_save(HttpServletRequest request, HttpServletResponse response, String id, String ztc_status,
		...
		mv.addObject("list_url", CommUtil.getURL(request) + "/admin/sms_set.htm");
		return mv;
	}
	...
	public ModelAndView sms_gold_log(HttpServletRequest request, HttpServletResponse response, String log_type,
			String currentPage, String store_name, String log_status) {
		ModelAndView mv = new JModelAndView("admin/blue/sms_gold_log.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		if (!StringUtils.isNullOrEmpty(log_status)) {
			qo.addQuery("obj.log_status", new SysMap("log_status", CommUtil.null2Int(log_status)), "=");
			mv.addObject("log_status", log_status);
		}
		if (!StringUtils.isNullOrEmpty(log_type)) {
