...
public class SubjectManageAction {
	...
	public ModelAndView subject_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String ifctype) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		SubjectQueryObject qo = new SubjectQueryObject(currentPage, mv, "sequence", "asc");
			qo.addQuery("obj.ifcType", new SysMap("ifcType", "PC"), "=");
		if (!"".equals(CommUtil.null2String(ifctype))) {
			if("H5".equals(ifctype)){
				mv = new JModelAndView("admin/blue/subject_list_wap.html", configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 0, request, response);
					qo.addQuery("obj.ifcType", new SysMap("ifcType", "H5"), "=");
			}
		}
	...
	public ModelAndView subject_add(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView subject_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (id != null && !StringUtils.isNullOrEmpty(id)) {
			Subject subject = this.subjectService.getObjById(Long.parseLong(id));
	...
	public ModelAndView subject_save(HttpServletRequest request, HttpServletResponse response, String id, String type) {
		WebForm wf = new WebForm();
		Subject subject = null;
		if (StringUtils.isNullOrEmpty(id)) {
			subject = wf.toPo(request, Subject.class);
			subject.setAddTime(new Date());
			subject.setIfcType(type);
		} else {
		...
		try {
			...
			if (StringUtils.isNullOrEmpty(fileName)) {
				if (!StringUtils.isNullOrEmpty(map.get("fileName"))) {
					...
					photo.setPath(this.ftpTools.systemUpload(photo.getName(), "/subject"));
		...
		ModelAndView mv = new JModelAndView("admin/blue/success.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("list_url", CommUtil.getURL(request) + "/admin/subject_list.htm");
	...
	public String subject_del(HttpServletRequest request, HttpServletResponse response, String mulitId, String currentPage) {
	...
	public void subject_list_ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName,
		...
		for (Field field : fields) {
			if (field.getName().equals(fieldName)) {
				...
				if ("displayAPP20".equals(field.getName())) {
					if (obj.getDisplayAPP20() == 1) {
						obj.setDisplayAPP20(0);
						val = obj.getDisplayAPP20();
					} else {
						obj.setDisplayAPP20(1);
						val = obj.getDisplayAPP20();
					}
				}
	...
	public ModelAndView subject_set(HttpServletRequest request, HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_set.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public void subject_img_upload(HttpServletRequest request, HttpServletResponse response, String id, String img_id) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		// 品牌标识图片
		String uploadFilePath = this.configService.getSysConfig().getUploadFilePath();
		String saveFilePathName = CommUtil.getServerRealPathFromRequest(request) + uploadFilePath + File.separator
		...
		Accessory img = null;
		if (img_id != null && !"undefined".equals(img_id)) {
		...
		try {
			...
			if (StringUtils.isNullOrEmpty(fileName)) {
				if (!StringUtils.isNullOrEmpty(map.get("fileName"))) {
					...
					if (!StringUtils.isNullOrEmpty(obj.getSubject_detail())) {
						subject_detail = (List<Map>) Json.fromJson(obj.getSubject_detail());
					}
					Map temp_map = new HashMap();
					temp_map.put("type", "img");
					temp_map.put("id", photo.getId());
					temp_map.put("img_url", photo.getPath() + "/" + photo.getName());
					subject_detail.add(temp_map);
					obj.setSubject_detail(Json.toJson(subject_detail, JsonFormat.compact()));
					this.subjectService.update(obj);
				}
			} else {
				if (!StringUtils.isNullOrEmpty(map.get("fileName"))) {
					...
					this.accessoryService.update(photo);
					if (obj.getSubject_detail() != null) {
						List<Map> subject_detail = (List<Map>) Json.fromJson(obj.getSubject_detail());
						for (Map temp_map : subject_detail) { // 更新json数据图片路径
							if (CommUtil.null2String(temp_map.get("id")).equals(photo.getId().toString())) {
								temp_map.put("img_url", photo.getPath() + "/" + photo.getName());
								break;
							}
						}
	...
	public void subject_img_href(HttpServletRequest request, HttpServletResponse response, String id, String img_id,
			String img_href) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		if (obj.getSubject_detail() != null) {
			List<Map> objs = (List<Map>) Json.fromJson(obj.getSubject_detail());
			for (Map temp_map : objs) {
				if ("img".equals(CommUtil.null2String(temp_map.get("type")))
						&& CommUtil.null2String(temp_map.get("id")).equals(img_id)) {
					if (!StringUtils.isNullOrEmpty(img_href)) { // 设置图片链接
	...
	public void subject_img_dele(HttpServletRequest request, HttpServletResponse response, String id, String dele_id,
			String type) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		if (obj.getSubject_detail() != null) {
			List<Map> subject_detail = (List<Map>) Json.fromJson(obj.getSubject_detail());
			...
			for (Map temp_map : subject_detail) {
				...
				if (CommUtil.null2String(temp_map.get("type")).equals(type)
						&& CommUtil.null2String(temp_map.get("id")).equals(dele_id)) {
	...
	public void subject_img_coords(HttpServletRequest request, HttpServletResponse response, String id, String img_id,
		...
		if (obj.getSubject_detail() != null) {
			...
			for (Map temp_map : objs) {
				if ("img".equals(CommUtil.null2String(temp_map.get("type")))
						&& CommUtil.null2String(temp_map.get("id")).equals(img_id)) {
					if (!StringUtils.isNullOrEmpty(coords) && !StringUtils.isNullOrEmpty(coords_url)) {
						...
						String areaInfo = "";
						if (!StringUtils.isNullOrEmpty(temp_map.get("areaInfo"))) {
							areaInfo = CommUtil.null2String(temp_map.get("areaInfo"));
	...
	public void subject_img_coords_dele(HttpServletRequest request, HttpServletResponse response, String coords, String url,
			String id) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		boolean ret = false;
		if (obj.getSubject_detail() != null) {
			...
			for (Map temp_map : subject_detail) {
				if (temp_map.get("areaInfo") != null) {
					String areaInfo = CommUtil.null2String(temp_map.get("areaInfo"));
	...
	public ModelAndView subject_gc(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_gc.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView subject_goods(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String id, String goods_ids) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_goods.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView subject_goods_load(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String gc_id, String goods_name) {
		ModelAndView mv = new JModelAndView("admin/blue/subject_goods_load.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/admin/subject_goods_load.htm", "",
	...
	public String subject_goods_save(HttpServletRequest request, HttpServletResponse response, String id, String ids,
			String old_ids) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		List<Map> maps = new ArrayList<Map>();
		if (!StringUtils.isNullOrEmpty(obj) && !StringUtils.isNullOrEmpty(obj.getSubject_detail())) {
			maps = (List<Map>) Json.fromJson(obj.getSubject_detail());
		}
		if (!StringUtils.isNullOrEmpty(old_ids)) {
			for (Map temp_map : maps) {
				if ("goods".equals(CommUtil.null2String(temp_map.get("type")))
						&& CommUtil.null2String(temp_map.get("goods_ids")).equals(old_ids)) {
	...
	public void subject_goods_dele(HttpServletRequest request, HttpServletResponse response, String id, String goods_ids,
			String type) {
		Subject obj = this.subjectService.getObjById(CommUtil.null2Long(id));
		if (obj.getSubject_detail() != null) {
			List<Map> subject_detail = (List<Map>) Json.fromJson(obj.getSubject_detail());
			...
			for (Map temp_map : subject_detail) {
				temp_maps.add(temp_map);
				if (CommUtil.null2String(temp_map.get("type")).equals(type)
						&& CommUtil.null2String(temp_map.get("goods_ids")).equals(goods_ids)) {
					temp_maps.remove(temp_map);
					break;
				}
			}
