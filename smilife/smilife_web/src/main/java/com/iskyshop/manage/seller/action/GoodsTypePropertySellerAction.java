...
public class GoodsTypePropertySellerAction {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy,
			String orderType) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/gtp_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		GoodsTypePropertyQueryObject qo = new GoodsTypePropertyQueryObject(currentPage, mv, orderBy, orderType);
	...
	public ModelAndView add(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/gtp_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		return mv;
	}
	...
	public String save(HttpServletRequest request, HttpServletResponse response, String count, String currentPage) {
		...
		for (int i = 1; i <= CommUtil.null2Int(count); i++) {
			...
			if (!StringUtils.isNullOrEmpty(name) && !StringUtils.isNullOrEmpty(value)) {
				...
				if (StringUtils.isNullOrEmpty(id)) {
					...
					gtp = this.gtpService.getObjById(Long.parseLong(id));
				}
		...
		request.getSession(true).setAttribute("url", CommUtil.getURL(request) + "/seller/gtp_list.htm");
		request.getSession(true).setAttribute("op_title", "添加成功");
	...
	public String delete(HttpServletRequest request, String mulitId, String currentPage) {
		...
		for (String id : ids) {
			if (!StringUtils.isNullOrEmpty(id)) {
				GoodsTypeProperty obj = this.gtpService.getObjById(Long.parseLong(id));
				if (obj != null) {
	...
	public void gtp_ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName, String value)
