...
public class StoreStatSellerAction {
	...
	@SecurityMapping(title = "店铺统计", value = "/seller/stat_store.htm*", rtype = "seller", rname = "店铺统计", rcode = "seller_stat_store", rgroup = "商家统计")
	...
	public ModelAndView stat_store(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/stat_store.html", configService.getSysConfig(),
		...
		List<GoodsLog> logs = this.goodsLogService.query(
				"select obj from GoodsLog obj where obj.store_id =:store_id and obj.addTime>=:beginDate and obj.addTime<:endDate",
				params, -1, -1);
		...
		for (GoodsLog gl : logs) {
			...
			if (!StringUtils.isNullOrEmpty(jsonStr)) {
				Map<String, Integer> fromMap = (Map<String, Integer>) Json.fromJson(jsonStr);
				...
				while (it.hasNext()) {
					...
					if (orderTypeMap.containsKey(from)) {
						orderTypeMap.put(from, orderTypeMap.get(from) + fromMap.get(key));
					} else {
		...
		while (it.hasNext()) {
			...
			clickFromsb.append("['").append(string).append("',").append(goods_clickfrom.get(string)).append("],");
		}
		...
		params.put("beginDate", begin);
		params.put("endDate", end);
		int storeFavCount = this.favoriteService
				.query("select obj.id from Favorite obj where obj.type =:type and obj.store_id =:store_id and obj.addTime>=:beginDate and obj.addTime<:endDate ",
						params, -1, -1)
				.size();
	...
	@SecurityMapping(title = "地域分布", value = "/seller/stat_formArea.htm*", rtype = "seller", rname = "店铺统计", rcode = "seller_stat_store", rgroup = "商家统计")
	@RequestMapping("/seller/stat_formArea.htm")
	public ModelAndView stat_formArea(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/stat_formArea.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		...
		Object formAreaDataTemp = this.orderFormService
				.query("select substring(receiver_area,1,2),count(*) from OrderForm obj where obj.order_form =:order_form and obj.store_id=:store_id "
						+ "and obj.order_status>=:order_status and obj.finishTime>=:beginDate and obj.finishTime<:endDate "
