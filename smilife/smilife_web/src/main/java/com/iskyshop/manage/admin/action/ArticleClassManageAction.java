...
public class ArticleClassManageAction {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response, String currentPage, String orderBy,
			String orderType) {
		ModelAndView mv = new JModelAndView("admin/blue/articleclass_list.html", configService.getSysConfig(),
	...
	public ModelAndView add(HttpServletRequest request, HttpServletResponse response, String pid) {
		...
		if (!StringUtils.isNullOrEmpty(pid)) {
			...
			obj.setParent(this.articleClassService.getObjById(Long.parseLong(pid)));
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response, String id, String pid,
		...
		if (StringUtils.isNullOrEmpty(id)) {
			...
			ArticleClass obj = this.articleClassService.getObjById(Long.parseLong(id));
			articleClass = (ArticleClass) wf.toPo(request, obj);
		}
		if (!StringUtils.isNullOrEmpty(pid)) {
			ArticleClass parent = this.articleClassService.getObjById(Long.parseLong(pid));
	...
	public String delete(HttpServletRequest request, String mulitId) {
		...
		for (String id : ids) {
			if (!StringUtils.isNullOrEmpty(id)) {
				Set<Long> list = this.genericIds(this.articleClassService.getObjById(Long.parseLong(id)));
				Map<String, Object> params = new HashMap<String, Object>();
				params.put("ids", list);
				List<ArticleClass> acs = this.articleClassService.query(
						"select obj from ArticleClass obj where obj.id in (:ids) order by obj.level desc", params, -1, -1);
	...
	public ModelAndView articleclass_data(HttpServletRequest request, HttpServletResponse response, String pid,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/articleclass_data.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Map map = new HashMap();
		map.put("pid", Long.parseLong(pid));
		List<ArticleClass> acs = this.articleClassService.query("select obj from ArticleClass obj where obj.parent.id =:pid",
				map, -1, -1);
		mv.addObject("acs", acs);
		mv.addObject("currentPage", currentPage);
		return mv;
	...
	public void articleclass_verify(HttpServletRequest request, HttpServletResponse response, String className, String id) {
		...
		List<ArticleClass> gcs = this.articleClassService
				.query("select obj from ArticleClass obj where obj.className=:className and obj.id!=:id", params, -1, -1);
