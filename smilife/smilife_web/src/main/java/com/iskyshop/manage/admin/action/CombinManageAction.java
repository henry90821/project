...
public class CombinManageAction {
	...
	public ModelAndView combin(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String combin_status, String beginTime, String endTime, String goods_name, String type, String combin_form) {
		ModelAndView mv = new JModelAndView("admin/blue/combin_goods.html", configService.getSysConfig(),
		...
		if (!StringUtils.isNullOrEmpty(combin_form)) {
			qo.addQuery("obj.combin_form", new SysMap("combin_form", CommUtil.null2Int(combin_form)), "=");
			mv.addObject("combin_form", combin_form);
		}
		if (!StringUtils.isNullOrEmpty(goods_name)) {
			qo.addQuery("obj.main_goods_name", new SysMap("goods_name", "%" + CommUtil.null2String(goods_name) + "%"),
					"like");
	...
	public String combin_goods_audit(HttpServletRequest request, HttpServletResponse response, String mulitId,
		...
		return "redirect:/admin/combin.htm?currentPage=" + currentPage + "&type=" + type;
	}
	...
	public String combin_goods_refuse(HttpServletRequest request, HttpServletResponse response, String mulitId,
		...
		for (String id : ids) {
			if (!StringUtils.isNullOrEmpty(id)) {
				...
				if (!StringUtils.isNullOrEmpty(refuse_msg)) {
