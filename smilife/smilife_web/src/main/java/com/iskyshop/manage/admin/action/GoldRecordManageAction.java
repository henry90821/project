 ...
 * 
 * <p>
 * Title: GoldRecordManageAction.java
 * </p>
 * 
 * <p>
 * Description: 系统金币管理类
...
public class GoldRecordManageAction {
	...
	public ModelAndView gold_record(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		ModelAndView mv = new JModelAndView("admin/blue/gold_record.html", configService.getSysConfig(),
		...
		if (this.configService.getSysConfig().isGold()) {
			...
			if (!StringUtils.isNullOrEmpty(beginCount)) {
				qo.addQuery("obj.gold_count", new SysMap("gold_count", CommUtil.null2Int(beginCount)), ">=");
			}
			if (!StringUtils.isNullOrEmpty(endCount)) {
				qo.addQuery("obj.gold_count", new SysMap("endCount", CommUtil.null2Int(endCount)), "<=");
			}
	...
	public ModelAndView gold_log(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		ModelAndView mv = new JModelAndView("admin/blue/gold_log.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (this.configService.getSysConfig().isGold()) {
	...
	public ModelAndView gold_record_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/gold_record_edit.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (this.configService.getSysConfig().isGold()) {
			if (!StringUtils.isNullOrEmpty(id)) {
				GoldRecord goldrecord = this.goldrecordService.getObjById(Long.parseLong(id));
	...
	public ModelAndView gold_record_save(HttpServletRequest request, HttpServletResponse response, String id,
		...
		if (this.configService.getSysConfig().isGold()) {
			...
			if (StringUtils.isNullOrEmpty(id)) {
				...
				GoldRecord obj = this.goldrecordService.getObjById(Long.parseLong(id));
	...
	public ModelAndView gold_record_view(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/gold_record_view.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (this.configService.getSysConfig().isGold()) {
			if (!StringUtils.isNullOrEmpty(id)) {
				GoldRecord goldrecord = this.goldrecordService.getObjById(Long.parseLong(id));
