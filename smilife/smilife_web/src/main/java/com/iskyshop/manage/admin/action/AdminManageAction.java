...
import com.iskyshop.manage.ftp.action.FTPServerManageAction;
...
public class AdminManageAction implements ServletContextAware {
	...
	public ModelAndView admin_list(String currentPage, String orderBy, String orderType, HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("admin/blue/admin_list.html", configService.getSysConfig(),
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/admin/admin_list.htm", "", "", pList, mv);
	...
	public ModelAndView admin_add(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/admin_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView admin_save(HttpServletRequest request, HttpServletResponse response, String id, String role_ids,
		...
		if (StringUtils.isNullOrEmpty(id)) {
			if(this.userService.getAdminByUsername(userName) != null) {//已存在对应管理员账号
				ModelAndView mv = new JModelAndView("admin/blue/error.html", configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
				if (add_url != null) {
					mv.addObject("add_url", add_url);
				}
				mv.addObject("list_url", list_url);
				mv.addObject("op_title", "已存在账号名为" + userName + "的管理员账号，添加管理员失败");
				return mv;
			}
			user = new User();
		...
		if(StringUtils.isNullOrEmpty(new_password)) {
			...
			List<Role> roles = this.roleService.query("select obj from Role obj where obj.display=:display and obj.type=:type", params, -1, -1);
			...
			List<FTPServer> FtpServers = this.ftpService.query(
					"select obj from FTPServer obj where obj.ftp_users.size<obj.ftp_amount and obj.ftp_type=:ftp_type", ftp_map,
					-1, -1);
			if (FtpServers.size() > 0) {
				...
				List<FTPServer> FtpServers2 = this.ftpService.query(
						"select obj from FTPServer obj where obj.ftp_type=:ftp_type order by obj.ftp_users.size asc", ftp_map,
						-1, -1);
	...
	public String admin_del(HttpServletRequest request, String mulitId, String currentPage) {
		...
		Long curUID = SecurityUserHolder.getCurrentUser().getId();
		for (String id : ids) {
			if (!"".equals(id)) {
				User user = this.userService.getObjById(Long.parseLong(id));
				if (!"admin".equalsIgnoreCase(user.getUsername()) && !curUID.equals(user.getId()) ) {
					// 删除管理员操作日志
					Map params = new HashMap();
					params.put("user_id", CommUtil.null2Long(id));
					List<SysLog> logs = this.syslogService.query("select new SysLog(id) from SysLog obj where obj.user_id=:user_id", params, -1, -1);
	...
	public ModelAndView admin_pws(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/admin_pws.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView admin_log_list(String currentPage, String orderBy, String orderType, HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/admin_log_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		SysLogQueryObject qo = new SysLogQueryObject(currentPage, mv, orderBy, orderType);
		qo.setPageSize(20);
	...
	public String admin_log_delete(String currentPage, HttpServletRequest request, HttpServletResponse response,
			String mulitId, String type) {
		if (type != null && "all".equals(type)) {
			List list = this.syslogService.query("select obj.id from SysLog obj order by obj.addTime asc", null, -1, -1);
			this.syslogService.batchDelete(list);
		} else {
			String[] ids = mulitId.split(",");
			for (String id : ids) {
	...
	public String init_role() {
		...
		if (current_user != null && current_user.getUserRole().indexOf("ADMIN") >= 0
				&& "admin".equals(current_user.getUsername())) {
			...
			clzs.add(SelfSeckillManageAction.class);
			...
			clzs.add(FTPServerManageAction.class);
			// 卖家权限加载
					...
					for (Method m : ms) {
						...
						for (Annotation tag : annotation) {
							if (SecurityMapping.class.isAssignableFrom(tag.annotationType())) {
								...
								List<Res> ress = this.resService.query("select obj from Res obj where obj.value=:value",
										params, -1, -1);
								if (ress.size() == 0) {
									Res res = new Res();
									...
									List<Role> roles = this.roleService
											.query("select obj from Role obj where obj.roleCode=:roleCode", params, -1, -1);
									Role role = null;
									if (roles.size() > 0) {
										role = roles.get(0);
									}
									...
									List<RoleGroup> rgs = this.roleGroupService.query(
											"select obj from RoleGroup obj where obj.name=:groupName and obj.type=:type",
											params, -1, -1);
									RoleGroup rg = null;
									if (rgs.size() > 0) {
										rg = rgs.get(0);
			...
			// 添加默认超级管理员并赋予所有权限
			User user = this.userService.getObjByProperty(null, "userName", "admin");
			Map params = new HashMap();
			...
			if (user == null) {
				...
				for (Role role : roles) {
					if (!"SELLER".equalsIgnoreCase(role.getType())) {
						user.getRoles().add(role);
					}
				}
				this.userService.save(user);
				...
				for (Role role : roles) {
					if (!"SELLER".equals(role.getType())) {
						logger.debug(role.getRoleName() + " " + role.getType() + " " + role.getRoleCode());
						user.getRoles().add(role);
					}
				}
				this.userService.update(user);
			...
			List<Role> admin_roles = this.roleService
					.query("select obj from Role obj where obj.display=:display and obj.type=:type", params, -1, -1);
			params.clear();
			params.put("type", "BUYER");
			List<Role> buyer_roles = this.roleService.query("select obj from Role obj where obj.type=:type", params, -1, -1);
			params.clear();
			params.put("userRole", "ADMIN");
			params.put("userName", "admin");
			List<User> admins = this.userService.query(
					"select obj from User obj where obj.userRole=:userRole and obj.userName!=:userName", params, -1, -1);
			for (User admin : admins) {//问题：执行此循环以后，所有非admin的管理员的权限都变得一样了
			...
			List<User> buyers = this.userService.query("select obj from User obj where obj.userRole=:userRole", params, -1,
					-1);
			this.userService.batchUpdate(buyers);
			...
			List<Role> seller_roles = this.roleService
					.query("select obj from Role obj where (obj.type=:type1 or obj.type=:type2)", params, -1, -1);
			params.clear();
			params.put("userRole0", "SELLER");
			params.put("userName", "admin");
			List<User> sellers = this.userService.query(
					"select obj from User obj where obj.userRole=:userRole0 and obj.userName!=:userName ", params, -1, -1);
			for (User seller : sellers) {
				if (seller.getStore() != null && seller.getStore().getStore_status() == 15) {// 商家店铺正常营业状态下，初始化权限时候才给该商家赋予权限
