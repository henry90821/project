...
public class SubjectTools {
	...
	public int getWidth(String str) {
		...
		int temp_width = CommUtil.null2Int(strs[0]) - CommUtil.null2Int(strs[2]);
		if (temp_width > 0) {
			width = temp_width;
		} else {
			width = 0 - temp_width;
	...
	public int getHeight(String str) {
		int height = 0;
		String strs[] = str.split(",");
		int temp_height = CommUtil.null2Int(strs[1]) - CommUtil.null2Int(strs[3]);
		if (temp_height > 0) {
			height = temp_height;
		} else {
	...
	public int getImageWidth(String id) {
		int width = 1640;
		Accessory image = this.accessoryService.getObjById(CommUtil.null2Long(id));
