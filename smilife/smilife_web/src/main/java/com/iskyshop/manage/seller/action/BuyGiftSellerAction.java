...
public class BuyGiftSellerAction {
	...
	public ModelAndView buygift_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
			String orderBy, String orderType, String gift_status, String beginTime, String endTime) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_list.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);

		Store store = SecurityUserHolder.getCurrentUser().getStore();
		String params = "";
		BuyGiftQueryObject qo = new BuyGiftQueryObject(currentPage, mv, orderBy, orderType);
		if (!StringUtils.isNullOrEmpty(gift_status)) {
			qo.addQuery("obj.gift_status", new SysMap("gift_status", CommUtil.null2Int(gift_status)), "=");
		}
		...
		if (!StringUtils.isNullOrEmpty(endTime)) {
			qo.addQuery("obj.endTime", new SysMap("endTime", CommUtil.formatDate(endTime)), "<=");
		}
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request) + "/seller/buygift_list.htm", "", params, pList, mv);
		return mv;
	}
	...
	public ModelAndView buygift_add(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_add.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		...
		if (store.getBuygift_meal_endTime() == null) {
			...
			mv.addObject("url", CommUtil.getURL(request) + "/seller/buygift_meal.htm");
			return mv;
		}
		...
		List<BuyGift> bgs = this.buygiftService.query(
				"select obj from BuyGift obj where obj.store_id=:sid and (obj.gift_status=10 or obj.gift_status=0)", params,
				-1, -1);
		if (bgs.size() > 0) {
			mv = new JModelAndView("user/default/sellercenter/seller_error.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			mv.addObject("op_title", "您当前有正在审核或进行的满就送");
			mv.addObject("url", CommUtil.getURL(request) + "/seller/buygift_list.htm");
			return mv;
		}
	...
	public ModelAndView buygift_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_edit.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(id)) {
			BuyGift buygift = this.buygiftService.getObjById(Long.parseLong(id));
			User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
			user = user.getParent() == null ? user : user.getParent();
			Store store = user.getStore();
			...
			List<Map> goodses = Json.fromJson(List.class, buygift.getGoods_info());
			for (Map map : goodses) {
	...
	public ModelAndView buygift_info(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_info.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (!StringUtils.isNullOrEmpty(id)) {
			BuyGift buygift = this.buygiftService.getObjById(Long.parseLong(id));
			User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
	...
	public void buygift_save(HttpServletRequest request, HttpServletResponse response, String id, String currentPage,
		...
		if (user.getStore().getBuygift_meal_endTime().before(new Date())) {
			...
			json.put("url", CommUtil.getURL(request) + "/seller/buygift_meal.htm");
			...
			boolean ret = true;
			List<BuyGift> bgs = this.buygiftService.query(
					"select obj from BuyGift obj where obj.store_id=:sid and (obj.gift_status=10 or obj.gift_status=0)",
					params, -1, -1);
			if (bgs.size() > 0) {
				json.put("op_title", "您当前有正在进行的满就送");
				json.put("url", CommUtil.getURL(request) + "/seller/buygift_list.htm");
				this.return_json(Json.toJson(json, JsonFormat.compact()), response);
			} else {
				...
				for (String vid : validate_ids) {
					Goods v_goods = this.goodsService.getObjById(CommUtil.null2Long(vid));
					String goods_store_id = v_goods.getGoods_store().getId().toString();
					String user_store_id = user.getStore().getId().toString();
					if (v_goods.getGoods_status() == 0 && v_goods.getGroup_buy() == 0
							&& v_goods.getOrder_enough_give_status() == 0 && v_goods.getOrder_enough_if_give() == 0
							&& v_goods.getEnough_reduce() == 0 && !goods_store_id.equals(user_store_id)) {
						ret = false;
					}
				}
				if (ret) {
					...
					for (Object goods_id : ids_set) {
						...
						goods.setBuyGift_amount(
								BigDecimal.valueOf(CommUtil.null2Double(request.getParameter("condition_amount"))));
						...
						String store_domainPath = CommUtil.getURL(request) + "/store_" + goods.getGoods_store().getId()
								+ ".htm";
	...
	public ModelAndView buy_goods_seller(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buy_goods_seller.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView buy_gift_seller(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buy_gift_seller.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		List<GoodsClass> gcs = this.goodsClassService
				.query("select obj from GoodsClass obj where obj.parent.id is null order by obj.sequence asc", null, -1, -1);
		mv.addObject("gcs", gcs);
	...
	public void buy_goods_seller_load(HttpServletRequest request, HttpServletResponse response, String goods_name,
			String gc_id, String goods_ids) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		boolean ret = true;
		Map params = new HashMap();
		...
		for (Goods obj : goods) {
			if (!ids_list.contains(obj.getId().toString())) {
				...
				if (obj.getGoods_main_photo() != null) {
					...
					map.put("img", this.configService.getSysConfig().getGoodsImage().getPath() + "/"
	...
	public ModelAndView buygift_meal(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_meal.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
	...
	public void buygift_meal_save(HttpServletRequest request, HttpServletResponse response, String meal_day)
				...
				if (day != null) {
					if (day.after(new Date())) {
						user.getStore().setBuygift_meal_endTime(
								this.addDate(user.getStore().getBuygift_meal_endTime(), CommUtil.null2Long(days)));
						...
						c_log.setBegin_time(user.getStore().getBuygift_meal_endTime());
						c_log.setEnd_time(this.addDate(user.getStore().getBuygift_meal_endTime(), CommUtil.null2Long(days)));
						...
						user.getStore().setBuygift_meal_endTime(CommUtil.formatDate(latertime, "yyyy-MM-dd HH:mm:ss"));
						this.storeService.update(user.getStore());
						// 记录金币日志
						GoldLog log = new GoldLog();
						log.setAddTime(new Date());
	...
	public ModelAndView buygift_meal_log(HttpServletRequest request, HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/buygift_meal_log.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		Store store = user.getStore();
	...
	public void gift_count_adjust(HttpServletRequest request, HttpServletResponse response, String gid, String count) {
	...
	public String buygift_del(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
	...
	public String buygift_stop(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
		User user = this.userService.getObjById(SecurityUserHolder.getCurrentUser().getId());
		user = user.getParent() == null ? user : user.getParent();
		BuyGift bg = this.buygiftService.getObjById(CommUtil.null2Long(id));
		if (bg != null && bg.getStore_id().equals(user.getStore().getId()) && bg.getGift_status() == 10) {
			...
			for (Map map : maps) {
				Goods goods = this.goodsService.getObjById(CommUtil.null2Long(map.get("goods_id")));
				if (goods != null) {
					if(!StringUtils.isNullOrEmpty(map.get("storegoods_count"))){//赠品
						int storegoods_count = CommUtil.null2Int(map.get("storegoods_count"));
						if (storegoods_count == 0) {//非共享库存时需要将剩余赠送库存增加到goods_inventory中去
