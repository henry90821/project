...
public class GoodsStatSellerAction {
	...
	@SecurityMapping(title = "商品统计", value = "/seller/stat_goods.htm*", rtype = "seller", rname = "商品统计", rcode = "seller_stat_goods", rgroup = "商家统计")
	...
	public ModelAndView stat_goods(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("user/default/sellercenter/stat_goods.html", configService.getSysConfig(),
	...
	@SecurityMapping(title = "有信息的商品", value = "/seller/stat_goods_list.htm*", rtype = "seller", rname = "商品统计", rcode = "seller_stat_goods", rgroup = "商家统计")
	...
	public ModelAndView stat_goods_list(HttpServletRequest request, HttpServletResponse response, String beginTime,
		...
		if (!StringUtils.isNullOrEmpty(endTime) && !StringUtils.isNullOrEmpty(beginTime)) {
			mv = new JModelAndView("user/default/sellercenter/stat_goods_list.html", configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 0, request, response);
			...
			String url = CommUtil.getURL(request) + "/seller/stat_goods_list.htm";
	...
	@SecurityMapping(title = "商品统计结果", value = "/seller/stat_goods_done.htm*", rtype = "seller", rname = "商品统计", rcode = "seller_stat_goods", rgroup = "商家统计")
	...
	public ModelAndView stat_goods_done(HttpServletRequest request, HttpServletResponse response, String beginTime,
		...
		if (!StringUtils.isNullOrEmpty(ids)) {
			...
			if (arr.length == 1) { // 一件商品的统计
				mv = new JModelAndView("user/default/sellercenter/stat_single_goods_result.html",
				...
				if (begin != null && end != null) {
					...
					while (!cal.getTime().after(end)) {
						...
						for (GoodsLog gl : log) {
							...
							if (!StringUtils.isNullOrEmpty(gl.getPreferential())) {
					...
					while (it.hasNext()) {
						String goodSpecType = (String) it.next();
						if (StringUtils.isNullOrEmpty(goodSpecType)) {
							specsb.append("['").append("默认规格").append("',").append(goods_spectype.get(goodSpecType))
									.append("],");
						} else {

							specsb.append("['").append(goodSpecType).append("',").append(goods_spectype.get(goodSpecType))
									.append("],");
						}
					}
				...
				mv = new JModelAndView("user/default/sellercenter/stat_multi_goods_result.html",
						configService.getSysConfig(), this.userConfigService.getUserConfig(), 0, request, response);
				if (begin != null && end != null) {
					Calendar cal = Calendar.getInstance();
					cal.setTime(begin);

					Map<Long, String> goods = new HashMap<Long, String>();
					...
					StringBuilder clicksb = new StringBuilder();
					while (it.hasNext()) {
						String cf = (String) it.next();
						clicksb.append("['").append(cf).append("',").append(goods_clickfrom.get(cf)).append("],");
					}
	...
	public ModelAndView stat_all(HttpServletRequest request, HttpServletResponse response, String beginTime, String endTime,
		...
		mv = new JModelAndView("user/default/sellercenter/stat_all_goods_result.html", configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if (begin != null && end != null) {
