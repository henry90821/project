...
public class Generator {
	...
	private void addJavaFiles(String bea) {
		...
		javaMapping.put("actionTemplate.java", "manage/action/" + bea + "ManageAction.java");
		javaMapping.put("daotemplate.java", "dao/" + bea + "DAO.java");
		javaMapping.put("IserviceTemplate.java", "service/" + "I" + bea + "Service.java");
		javaMapping.put("queryObjectTemplate.java", "domain/query/" + bea + "QueryObject.java");
	...
	private void mergeJava(String sourcePath, String targetPath) throws Exception {
		for (String templateFile : this.javafiles.keySet()) {
			...
			sw.close();
			outStream.close();
			logger.debug("成功生成Java文件:" + (targetPath + targetFile).replaceAll("/", "\\\\"));
		}
	}

	private void mergeHTML(String sourcePath, String targetPath) throws Exception {
		for (String templateFile : this.htmlfiles.keySet()) {
			...
			context.put("domainName", this.beanName.substring(beanName.lastIndexOf(".") + 1));
			FileOutputStream outStream = new FileOutputStream(new File(targetPath, targetFile));
			OutputStreamWriter writer = new OutputStreamWriter(outStream, "UTF-8");
			BufferedWriter sw = new BufferedWriter(writer);
			t.merge(context, sw);
			sw.flush();
			sw.close();
			outStream.close();
			logger.debug("成功生成页面文件:" + targetPath + targetFile);
		}
	}
	...
	public void generate() {
		this.init();
		try {
			String sourcePath = System.getProperty("iskyshopb2b2c.root") + this.templateDir;
			String html_targetPath = System.getProperty("iskyshopb2b2c.root") + this.resultDir;
			String java_targetPath = System.getProperty("iskyshopb2b2c.root") + this.resultDir + "\\"
					+ this.packageName.replace(".", "\\");
			logger.info("开始生成页面文件......");
			// this.mergeHTML(sourcePath, html_targetPath);
			this.mergeJava(sourcePath, java_targetPath);
		} catch (Exception e) {
			logger.error("生成HTML页面模板失败，失败原因如下:\n" + e);
		}
	}
}
